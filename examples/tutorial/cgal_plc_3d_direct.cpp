...
int main()
{
    ...
    viennagrid::projection_matrix( plc_points_3d.begin(), plc_points_3d.end(), 1e-6, center, projection_matrix );

    plc_points_2d.resize( plc_points_3d.size() );
    viennagrid::project( plc_points_3d.begin(), plc_points_3d.end(), plc_points_2d.begin(), center, (point_type*)projection_matrix, projection_matrix + 2 );
...
//     vertex_range_type vertices = viennagrid::elements<viennagrid::vertex_tag>(plc);
    ...
    viennagrid::project( vgrid_list_of_holes.begin(), vgrid_list_of_holes.end(), vgrid_list_of_holes_2d.begin(), center, (point_type*)projection_matrix, projection_matrix + 2 );
    ...
    typedef viennagrid::triangular_3d_mesh triangle_mesh_type;
        ...
        if(fit->is_in_domain())
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (pit == points.end())
                {
                    ...
                    vgrid_vtx[i] = viennagrid::make_vertex( triangle_mesh, tmp );
    ...
    vtk_writer(triangle_mesh, "test_3d");
