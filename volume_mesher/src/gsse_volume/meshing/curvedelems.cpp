...
// ----------------------------------------------------------------------------
	...
	// (std::cout) << "eshape = " << eshape << ", edshape = " << edshape << endl;
...
//	    fe2d->CalcEdgeShapes (x != NULL, dxdxi != NULL);
						      ...
						      Point<3> * x, Mat<3,3> * dxdxi)
  {
	...
	/*
	    std::cout << " Curved Element " << elnr << endl; 
	    std::cout << " hpref_el.coarse_elnr " << hpref_el.coarse_elnr << endl; 
	    std::cout << " hpref_el.param = " << endl; 
	    for(int j=0;j< hpref_el.np; j++) 
	      { 
		for(int k=0;k<3;k++)
		  std::cout << hpref_el.param[j][k] << "\t"; 
		std::cout << endl; 
	      } 
	*/
	...
	Point<3> coarse_xi(0,0,0);
	  ...
	  std::cout << " x " << x << endl;
	  // << "\t" << x.X(2) << "\t" << x.X(3) << endl;  
	  std::cout << " Element Trafo " << coarse_xi(0) << " \t " << coarse_xi(1) << " \t " << coarse_xi(2) << endl;
	  */
