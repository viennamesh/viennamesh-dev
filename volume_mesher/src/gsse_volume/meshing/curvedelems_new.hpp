...
public:
  ...
  bool IsHighOrder() const { return true; }
  void SetHighOrder () { ; }

  void BuildCurvedElements(Refinement * ref, int aorder);
...
private:
  ...
  public:
    ...
    int nv;
    int ndof;
    ArrayMem<int,12> edgenrs;
    ArrayMem<int,6> facenrs;
  };


  void CalcElementShapes (ElementInfo & info, const Point<3> & xi, Vector & shapes) const;
  void GetCoefficients (ElementInfo & info, ARRAY<Vec<3> > & coefs) const;
  void CalcElementDShapes (ElementInfo & info, const Point<3> & xi, DenseMatrix & dshapes) const;
