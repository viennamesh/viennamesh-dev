...
namespace netgen
{
  ...
  */
  ...
  void AdFront2 :: PrintOpenSegments (ostream & ost) const
  {
	...
	for (i = 1; i <= lines.Size(); i++)
	  if (lines.Get(i).Valid())
	    ost << i << ": " << GetGlobalIndex (lines.Get(i).L().I1()) << "-"
			      ...
			      bool pointonsurface)
  {
	...
	points.Elem(pi) = FrontPoint2 (p, globind, mgi, pointonsurface);
      }
    else
      {
	pi = points.Append (FrontPoint2 (p, globind, mgi, pointonsurface));
      }
			     ...
			     const PointGeomInfo & gi1, const PointGeomInfo & gi2)
  {
    ...
    /*
      std::cout << "add front line: " << p1.P() << " - " << p2.P()
    ...
    */
	...
	if (allflines->Used (INDEX_2 (GetGlobalIndex (pi1), 
				      GetGlobalIndex (pi2))))
	  {
	    ...
	    std::cout << "ERROR Adfront2::AddLine: line exists" << endl;
	  }
			     ...
			     double xh)
  {
    ...
    invpindex.SetSize (points.Size()); invpindex = -1;
	...
	if (points.Get(i).Valid() && 
	    ...
	    invpindex.Get(i) <= 0)
	  {
	    ...
	    pindex.Append(i);
	    //std::cout << " appended"<< endl;
	  }
	//else
	// std::cout << " not appended p0 " << p0 << " xh " << xh << " invpindex " << invpindex.Get(i) << endl;
      }
