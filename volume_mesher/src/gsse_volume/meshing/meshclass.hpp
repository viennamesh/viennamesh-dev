...
public:
  ...
  */
  ...
  /// Calculates localh 
  void CalcLocalH (long threadID);
  ///
  void SetLocalH (const Point3d & pmin, const Point3d & pmax, double grading, long threadID);
  ///
...
//  { return *clusters; }
  ...
  /// loads the mastermesh and distributes it to local meshes
  void LoadAndDistribute (istream & infile);

  /// loads a mesh sent from master processor
  void LoadParallelMesh ( );
...
//   void FindExchangeFaces ();
  /// use metis to split master mesh 
  void ParallelMetis ( Mesh * mastermesh, ARRAY<int> & neloc );

  /// send the master mesh to parallel processors
  void SendMesh ( Mesh * mastermesh, ARRAY<int> & neloc, const double & scale, 
		  const int & inverttets, const int & invertsurf );
  ...
  // [RH] extension
  void get_elements_on_vertex(int vertex_number, ARRAY<int>& element_container);
