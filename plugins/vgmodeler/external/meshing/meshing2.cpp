...
#ifdef DEBUGALL
    std::cout << "..in Meshing2 constructor: " 
// 	      <<" aboundingbox[0]: " << boundingbox[0]
// 	      <<" aboundingbox[1]: " << boundingbox[1] 
// 	      <<" aboundingbox[2]: " << boundingbox[2] 
	      << std::endl;
#endif
  ...
  {
    std::cout << ".. DefineTransformation .. " << std::endl;
  ...
  {
    std::cout << ".. TransformToPlain .. " << std::endl;
  ...
  {
#ifdef DEBUGALL      
    std::cout << "[FS] .. in Meshing2::GenerateMesh .. " << std::endl;
    std::cout << ".. facenr: " << facenr << std::endl;
     std::cout << "..mesh: " << mesh << std::endl;

     std::string filename = "fs_surface_temp_"+ boost::lexical_cast<std::string>(facenr) +".vol";
     mesh.Save(filename);

     std::cout << "..mesh.GetNSE: " << mesh.GetNSE() << std::endl;
//      std::cout << "..mesh.GetH: " << mesh.GetH() << std::endl;
//      std::cout << "..mesh.GetMinH: " << mesh.GetMinH() << std::endl;
//      std::cout << "..mesh.GetNSE: " << mesh.GetNSE() << std::endl;
//      std::cout << "..mesh.GetNSE: " << mesh.GetNSE() << std::endl;
#endif
...
#ifdef DEBUGALL      
    std::cout << "..in meshing2 number of surface elements ..: " << mesh.GetNSE() << std::endl;
#endif
...
#ifdef DEBUGALL      
    std::cout << ".. meshedarea_before: " << meshedarea_before << std::endl;
#endif
...
// 	    std::string blabla;
// 	    std::cout << "==============================================================================================================" << std::endl;
// 	    std::cout << ".. wait for input in GenerateMesh2D .. =======================================================================" << std::endl;
// 	    cin >> blabla;
// 	  }
...
#ifdef DEBUGALL      	
	std::cout << "..adfront nfaces: " << nfaces << std::endl;
#endif
...
#ifdef DEBUGALL      
        std::cout << "baseline = " << baselineindex << ", p1, p2 = " << p1 << ", " << p2 << endl;
        std::cout << "..blgeominfo1: " << blgeominfo1->trignum << " :: blgeominfo2: " << blgeominfo2->trignum << std::endl;
#endif
...
// 	std::cout << ".. lfs: " << hshould << " .. gh: " << gh << std::endl;

	// [FS] .. currently do not know if this function is needed - it was commented out
	//
...
#ifdef DEBUGALL      
        std::cout << "locpoints: " << locpoints << std::endl;
        std::cout << "loclines : " << loclines << std::endl;
#endif
...
#ifdef DEBUGALL      
        std::cout << ".. loclines[0]..points of first line: p1: " << gpi1 << " :: p2: " << gpi2 << std::endl;
#endif
...
// 	    if (debugflag)
#ifdef DEBUGALL      
            std::cout << "define new transformation" << std::endl;
#endif
...
#ifdef DEBUGALL      
                std::cout << "transform " << locpoints.Get(i) << " to " << plainpoints.Get(i).X() << " " << plainpoints.Get(i).Y() << std::endl;
                std::cout << "plainzones.Elem("<< i <<"): " << plainzones.Elem(i) << std::endl;
#endif
	      }
	    ...
	    */
	    ...
	    for (int i = 2; i <= loclines.Size(); i++)  // don't remove first line
	      {
#ifdef DEBUGALL      
                 std::cout << "loclines(i) = " << loclines.Get(i).I1() << " - " << loclines.Get(i).I2() << endl;
                 std::cout << "loclines("<< i <<")..global points: p1: " << adfront -> GetGlobalIndex (pindex.Get(loclines[i-1].I1()))
                           << " :: p2: " << adfront -> GetGlobalIndex (pindex.Get(loclines[i-1].I2())) << std::endl;
#endif
...
#ifdef DEBUGALL      
                std::cout << "..plainzones: p1: " << z1 << " :: p2: " << z2 << std::endl;
#endif
		...
		if ( (z1 >= 0) != (z2 >= 0))
		  {
			...
			if (!morerisc)
			  {
			    ...
			    double len = v.Length();
			    if (len <= 1e-6)
                               std::cout << "WARNING(js): inner-outer: short vector" << std::endl;
			    else
			      v /= len;
...
#ifdef DEBUGALL      
                        std::cout << "inner - outer correction" << std::endl;
#endif
		      }
		    else
		      {
			// remove line
#ifdef DEBUGALL      
                         std::cout << ".. in else: delete loclines["<< i << "] .. " << std::endl;
#endif
			loclines.DeleteElement(i);
			lindex.DeleteElement(i);
			oldnl--;
		  ...
		  {
#ifdef DEBUGALL      
                     std::cout << ".. in else if: delete loclines["<< i << "] .. " << std::endl;
#endif
		    loclines.DeleteElement(i);
		    lindex.DeleteElement(i);
		    oldnl--;
...
#ifdef DEBUGALL      
                    std::cout << "..plainzones.Elem("<< i <<") < 0 : " << plainzones.Elem(i) << std::endl;
#endif
		  }
		if (pindex.Elem(i) == -1)
		  {
#ifdef DEBUGALL      
                     std::cout << "..pindex.Elem("<< i <<") == -1: " << pindex.Elem(i) << std::endl;
#endif
		    legalpoints.Elem(i) = 0;
		  }
		...
		if (plainpoints.Elem(i).Y() < -1e-10*avy) // changed
		  {
#ifdef DEBUGALL      
                     std::cout << "..plainpoints.Elem("<< i <<").Y() < -1e-10*avg: " << plainpoints.Elem(i).Y() << std::endl;
#endif
		    legalpoints.Elem(i) = 0;
		  }
	      }
...
#ifdef DEBUGALL      
            std::cout << "..maxlegalpoint: " << maxlegalpoint << std::endl;
            std::cout << "..maxlegalline: " << maxlegalline << std::endl;
#endif
...
#ifdef DEBUGALL      
            std::cout << "rule Nr = " << rulenr << std::endl;
#endif
...
#ifdef DEBUGALL      
		std::cout << "plain: " << plainpoints.Elem(i) << " => " << locpoints.Elem(i) << std::endl;
#endif
	...
	if (found && mparam.checkoverlap)
	  {
...
#ifdef DEBUGALL      
	    std::cout << " .. checkoverlap1: hullmin: " << hullmin << " :: hullmax: " << hullmax << std::endl;
#endif

	    surfeltree.GetIntersecting (hullmin, hullmax, intersecttrias);

#ifdef DEBUGALL      
            std::cout << "..intersecttrias: " << intersecttrias << std::endl;
#endif
...
#ifdef DEBUGALL      
	    std::cout << " .. checkoverlap2" << std::endl;
#endif
...
#ifdef DEBUGALL      
		std::cout << " .. checkoverlap 3" << std::endl;
#endif
		...
		*/
		...
		for (jj = 1; jj <= intersecttrias.Size(); jj++)
		  {
#ifdef DEBUGALL      
		    std::cout << " .. checkoverlap 3 0" << std::endl;
#endif
...
#ifdef DEBUGALL      
		    std::cout << " .. checkoverlap 3 0 1" << std::endl;
#endif
		    ...
		    for (jl = 1; jl <= ntrig; jl++)
		      {
#ifdef DEBUGALL      
			std::cout << " .. checkoverlap 3 0 2: ntrig: " << ntrig << std::endl;
#endif
...
#ifdef DEBUGALL      
			std::cout << " .. checkoverlap 3 0 3" << std::endl;
#endif
...
#ifdef DEBUGALL      
			std::cout << " .. checkoverlap 3 0 4" << std::endl;
#endif
...
#ifdef DEBUGALL      
			std::cout << " .. checkoverlap 3 1" << std::endl;
#endif
...
#ifdef DEBUGALL      
			std::cout << " .. checkoverlap 3 2" << std::endl;
#endif
...
#ifdef DEBUGALL      
	std::cout << " .. checkoverlap 4: found: " << found << std::endl;
#endif
...
#ifdef DEBUGALL      
	std::cout << " .. checkoverlap 5: found: " << found << std::endl;
#endif
	...
	if (found)
	  {
		...
		*/		  
...
#ifdef DEBUGALL      		
		std::cout << ".. adding surface element ..: " << mtri << std::endl;
#endif
		...
		if(maxarea > 0 && meshedarea-meshedarea_before > maxarea)
		  {
		    std::cout << "meshed area = " << meshedarea-meshedarea_before << endl
...
#ifdef DEBUGALL      
    std::cout << "[FS] .. leaving Meshing2::GenerateMesh .. " << std::endl;
#endif
