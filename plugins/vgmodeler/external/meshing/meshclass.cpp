...
//                 seg.geominfo[0].trignum = 1;
//                 seg.geominfo[1].trignum = 1;
//                 seg.surfnr1 = 0;
//                 seg.surfnr2 = 0;
//                 seg.edgenr = 1;
//                 seg.epgeominfo[0].dist = 0;
//                 seg.epgeominfo[1].edgenr = 1;
//                 seg.epgeominfo[1].dist = 0;
...
#ifdef DEBUGALL      
    std::cout << "[FS] .. in FindOpenSegments .. on surfnr: " << surfnr << std::endl;
#endif
...
//     INDEX_2_HASHTABLE<INDEX_2> faceht(4 * GetNSE()+GetNSeg()+1);   
    ...
    // [FS] .. separation of segments
    //
    INDEX_3_HASHTABLE<INDEX_3> faceht(4 * GetNSE()+GetNSeg()+1);   
  
#ifdef DEBUGALL      
    std::cout << ".. GetNSeg: " << GetNSeg() << ".. GetNSE(): " << GetNSE() << std::endl;
	...
	if (surfnr == 0 || seg.si == surfnr)
	  {
	    // [FS] .. old segment test
 	    INDEX_3 key(seg.p1, seg.p2, seg.si);
 	    INDEX_3 data(seg.si, -i, 0);
// 	    INDEX_2 key(seg.p1, seg.p2);
// 	    INDEX_2 data(seg.si, -i);

// 	    std::cout << "..segment points: p1: " << Point(seg.p1) << std::endl;

	    // [FS][TEST] .. interface test - mark edges from the interface facets as already meshed
	    //
// 	    if(seg.si == 9)
// 	      data.I1() = 0;
	    ...
	    if (faceht.Used (key))
	      {
// 		cerr << "ERROR: Segment " << seg 
// 		     << " :: p1: " << Point(seg.p1) 
// 		     << " :: p2: " << Point(seg.p2) 
// 		     << " already used" << endl;

#ifdef DEBUGALL      
		std::cout << "ERROR: Segment " << seg 
			  << " :: p1: " << Point(seg.p1) 
			  << " :: p2: " << Point(seg.p2) 		  
			  << " already used" << endl;
#endif
...
// 	    std::cout << "..setting faceht with: " << key << " and " << data << std::endl;
#ifdef DEBUGALL      
		std::cout << "..setting segment " << seg
// 			  << " :: p1: " << Point(seg.p1) 
// 			  << " :: p2: " << Point(seg.p2) 
			  << std::endl;
#endif
    ...
    // [FS] .. commented out .. not necessary
//     for (i = 1; i <= GetNSeg(); i++)
//       {
// 	const Segment & seg = LineSegment (i);
...
// 	if (surfnr == 0 || seg.si == surfnr)
// 	  {
// 	    // [FS][TODO] .. old segment test - still needed ?
//   	    INDEX_3 key(seg.p2, seg.p1, seg.si);
...
#ifdef DEBUGALL      
    std::cout << ".. number of surface elements: " << GetNSE() << std::endl;
#endif
	...
	if (surfnr == 0 || el.GetIndex() == surfnr)
	  {
	    for (j = 1; j <= el.GetNP(); j++)
	      {
		// [FS] .. INDEX = int
		//         INDEX_2 object for accessing an array of 2
		//
 		INDEX_3 seg (el.PNumMod(j), el.PNumMod(j+1), el.index);
		INDEX_3 data;
...
// 		INDEX_2 data;
...
#ifdef DEBUGALL      
		std::cout << "..check if seg: " << seg.I1() << " - " << seg.I2() << " is used" << std::endl;
#endif
		...
		if (faceht.Used(seg))
		  {
...
#ifdef DEBUGALL      
		    std::cout << "..data: " << data << " :: el.GetIndex(): " << el.GetIndex() << std::endl;
#endif
			...
			faceht.Set (seg, data);
		      ...
		      {
#ifdef DEBUGALL      
                         std::cout << "hash table si not fitting for segment: " 
                                   << seg.I1() << "-" << seg.I2() << " other = " << data.I2() << std::endl;;
#endif
		      }
		  }
		else
		  {
#ifdef DEBUGALL      
                     std::cout << ".. in else .. inverting I1 and I2 .. adding seg: " << seg << std::endl;
#endif
...
#ifdef DEBUGALL      
    std::cout << "faceht.GetNBags: " << faceht.GetNBags() << endl;
#endif
    ...
    for (i = 1; i <= faceht.GetNBags(); i++)
      {
#ifdef DEBUGALL      
	std::cout << "faceht.GetBagSize(" << i <<"): " << faceht.GetBagSize(i) << endl;
#endif
...
// 	  INDEX_2 data;
...
#ifdef DEBUGALL      
		  std::cout << " seg.geominfo[0]: " << seg.geominfo[0] << " :: geominfo[1]: " << seg.geominfo[1] << " ";
		  std::cout << "line seg: ";
#endif
		}
    ...
    std::cout << " open segments found: " << opensegments.Size() << std::endl;
#endif
...
#ifdef DEBUGALL      
    std::cout << "[FS] .. leaving FindOpenSegments .. " << std::endl;
#endif
  }
  ...
  void Mesh :: FreeOpenElementsEnvironment (int layers)
  {
    ...
    std::cout << "free: " << cntfree << ", fixed: " << GetNE()-cntfree << endl;
...
#ifdef DEBUGALL
    std::cout << "restrict h in " << p << " to " << hloc << endl;
#endif
  ...
  {
//     std::cout << ".. ENTER Mesh::GetH .. " << std::endl;
...
// 	std::cout << ".. lochfunc returned: " << hl << " :: hmin: " << hmin << std::endl;
      }
    
    // [FS] .. LFS test
    //
//     hmin = 0.125;
//     std::cout << ".. [FS] .. setting hmin to: " << hmin << std::endl;

//     std::cout << ".. EXIT Mesh::GetH .. " << std::endl;
    return hmin;
  }
