...
{
#ifdef DEBUGALL
   std::cout << " .. in Meshing2::ApplyRules .. " << std::endl;
#endif
  ...
  if (loctestmode)
    {
      std::cout << endl << endl << "Check new environment" << endl;
      std::cout << "tolerance = " << tolerance << endl;
      for (i = 1; i <= lpoints.Size(); i++)
	std::cout << "P" << i << " = " << lpoints.Get(i) << endl;
      std::cout << endl;
      for (i = 1; i <= llines.Size(); i++)
	std::cout << "(" << llines.Get(i).I1() << "-" << llines.Get(i).I2() << ")" << endl;
    }
#endif
...
#ifdef DEBUGALL
	  if(loctestmode)
	    std::cout << "..nlok: " << nlok << " .. rule->GetNoldL() " << rule->GetNOldL() << std::endl;
#endif
...
#ifdef DEBUGALL
		  if(loctestmode)
		    std::cout << "..lnearness.Get(locli): " << lnearness.Get(locli) << " .. rule->GetLNearness(nlok): " << rule->GetLNearness(nlok) << std::endl;
#endif
		      ...
		      for (int j = 0; j < 2 && ok; j++)
			{
			  ...
			  if (pmap.Get(refpi) != 0)
				  ...
				  || pused.Get(loclin[j]))
				{
				  ...
				  if(loctestmode)
				    {
				      std::cout << "nok pos3" << endl;
				      //if(rule->CalcPointDist (refpi, lpoints.Get(loclin[j])) > maxerr)
				      //std::cout << "r1" << endl;
				      //if(!legalpoints.Get(loclin[j]))
				      //std::cout << "r2 legalpoints " << legalpoints << " loclin " << loclin << " j " << j << endl;
		  ...
		  if (npok <= rule->GetNOldP())
...
#ifdef DEBUGALL
		      if (loctestmode)
			std::cout << "lines and points mapped" << endl;
#endif
...
#ifdef DEBUGALL
			      if (loctestmode)
				std::cout << "Orientation " << i << " not ok" << endl;
#endif
			    }
			}
...
#ifdef DEBUGALL
			  if (loctestmode) 
			    std::cout << "freezone not convex" << endl;
#endif
			  ...
			  if (cnt % 100 == 0)
			    {
			      ...
			      std::cout << "freezone not convex, cnt = " << cnt << "; rule = " << rule->Name() << endl;
			      std::cout << "tol = " << tolerance << endl;
			      std::cout << "maxerr = " << maxerr << "; minerr = " << minelerr << endl;
			      std::cout << "freezone = " << rule->GetTransFreeZone() << endl;
			    }
			  */
			}
...
#ifdef DEBUGALL
			      if (loctestmode)
				std::cout << "Point " << i << " in freezone" << endl;
#endif
			    }
			}
		      ...
		      if (ok)
			{
...
#ifdef DEBUGALL
			      if (loctestmode)
				std::cout << "r " << rule->Name() << "bad = " << hf << endl;
#endif
			  ...
			  if (elerr < 0.99*minelerr)
				...
				{
				  std::cout << "rule = " << rule->Name() << endl;
				  std::cout << "class = " << tolerance << endl;
				  std::cout << "lpoints: " << endl;
				  for (i = 1; i <= lpoints.Size(); i++)
				    std::cout << i << ": " << lpoints.Get(i) << endl;
				  std::cout << "llines: " << endl;
				  for (i = 1; i <= llines.Size(); i++)
				    std::cout << i << ": " << llines.Get(i).I1() << " " << llines.Get(i).I2() << endl;

				  std::cout << "Freezone: ";
				  for (i = 1; i <= rule -> GetTransFreeZone().Size(); i++)
				    std::cout << i << ": " << rule->GetTransFreeZone().Get(i) << endl;
				}
#endif
