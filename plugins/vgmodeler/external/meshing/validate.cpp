...
namespace vgmnetgen
{
		      ...
		      const BitArray & isnewpoint)
  {
    ...
    for (ElementIndex i = 0; i < mesh.GetNE(); i++)
      pure_badness[i] = mesh.VolumeElement(i).CalcJacobianBadness (mesh.Points());
		  ...
		  const ARRAY<double> & pure_badness, double max_worsening)
  {
    ...
    double max_pure_badness = 0;
    for(int i=0; i<pure_badness.Size(); i++)
      if(pure_badness[i] > max_pure_badness)
	max_pure_badness = pure_badness[i];
    ...
    for (ElementIndex i = 0; i < mesh.GetNE(); i++)
      {
	double bad = mesh.VolumeElement(i).CalcJacobianBadness (mesh.Points());
	if (bad > 1e10 || 
	    (max_worsening > 0 && bad > max_pure_badness*max_worsening))
	  bad_elements.Append(i);
	  

	if(max_worsening > 0)
	  {
	    double actw = bad/max_pure_badness;
	    if(actw > worsening)
	      {
		worsening = actw;
		ind = i;
		       ...
		       const ARRAY< ARRAY<int,PointIndex::BASE>* > & idmaps)
  {
    ...
    Validate(mesh,bad_elements,pure_badness,0.1*(max_worsening-1.) + 1.); // -> larger working area
    ...
    cout << "worsening: " <<
      Validate(mesh,bad_elements,pure_badness,max_worsening) << endl;

    int auxnum=0;
    for(int i=1; i<=np; i++)
      if(working_points.Test(i))
	auxnum++;
    cout << "Percentage working points: " << 100.*double(auxnum)/np << endl;
    ...
    while ((facokedge < 1.-1e-8 || facokface < 1.-1e-8) && cnttrials < 100)
      {
	cout << "max. worsening " << max_worsening << endl;
	oldlamedge = lamedge;
	lamedge *= 6;
	if (lamedge > 2)
	  lamedge = 2;
	...
	if(1==1 || facokedge < 1.-1e-8)
	  {
		...
		cout << "lamedge = " << lamedge << endl;
		
		factryedge = lamedge + (1.-lamedge) * facokedge;
		cout << "trying: " << factryedge << endl;
		...
		if(facokedge < 1.-1e-8)
		  ...
		  Validate(mesh,bad_elements,pure_badness,-1);
		cout << bad_elements.Size() << " bad elements" << endl;
	      }
	    while (bad_elements.Size() > 0 && cnttrials < 100);
	  }
	...
	if(cnttrials < 100)
	  {
		...
		if(cnttrials % 10 == 0)
		  max_worsening *= 1.1;
		cout << "lamface = " << lamface << endl;
		
		factryface = lamface + (1.-lamface) * facokface;
		cout << "trying: " << factryface << endl;
	...
	//!
	if((facokedge < 1.-1e-8 || facokface < 1.-1e-8) && cnttrials < 100)
	  {
	    ...
	    Validate(mesh,bad_elements,pure_badness,0.1*(max_worsening-1.) + 1.);
    ...
    */
	...
	Validate(mesh,bad_elements,pure_badness,max_worsening);
	...
	for(int i=0; i<bad_elements.Size(); i++)
	  {
	    cout << "bad element:" << endl
		 ...
		 << mesh[bad_elements[i]][3] << ": " << mesh.Point(mesh[bad_elements[i]][3]) << endl;
