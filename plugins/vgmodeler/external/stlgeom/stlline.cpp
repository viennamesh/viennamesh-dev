...
// [FS][MOD] also go through all materials
//
int STLEdgeDataList :: GetNConfEdges() const
{
  int i, ne = geom.GetNTE();
  ...
  for (i = 1; i <= ne; i++)
    {
      for(int m=0; m<2; m++)
	{
	  if (geom.GetTopEdge(i).segment_info[m].status == ED_CONFIRMED)
	    cnt++;
	}
    }
  return cnt;
...
int STLEdgeDataList :: GetNConfCandEPP(int p, int material) const
{
  int i;
  int cnt = 0;
  for (i = 1; i <= GetNEPP(p); i++)
...
// 	  Get(GetEdgePP(p,i)).GetStatus() == ED_CONFIRMED)
      ...
      if (Get(GetEdgePP(p,i)).segment_info[material].status == ED_CANDIDATE ||
	  Get(GetEdgePP(p,i)).segment_info[material].status == ED_CONFIRMED)
	{
	  cnt++;
	}
    }
  ...
  : pts(), lefttrigs(), righttrigs()
{
  ...
  // [FS][HACK] remove
//   for(int m=0; m<geometry->material_size; m++)
//     facenums[m] = -1;
};
...
#ifdef DEBUGALL
          std::cout << "..i: " << pts.Get(i) << " ..i+1: " << pts.Get(i+1) << std::endl;
          std::cout << "..v: " << v << " ..relval: " << relval << std::endl;
          std::cout << "..relval*v: " << relval * v << std::endl;
#endif
			 ...
			 class Mesh& mesh)
{
  ...
  // [FS] for non manifold part
  for(int m=0; m<geometry->material_size; m++)
    {
      line->facenums[m] = facenums[m];
      line->segment_info[m].lefttrig  = segment_info[m].lefttrig;
      line->segment_info[m].righttrig = segment_info[m].righttrig;
      line->segment_info[m].facenum[0] = segment_info[m].facenum[0];
      line->segment_info[m].facenum[1] = segment_info[m].facenum[1];
      line->segment_info[m].orient[0] = -1;
      line->segment_info[m].orient[1] = -1;

#ifdef DEBUGALL
      std::cout << ".. facenums: " << line->facenums[m] << std::endl;
      std::cout << ".. trignums: " << segment_info[m].lefttrig << " :: " << segment_info[m].righttrig << std::endl;
      std::cout << ".. trignums: " << line->segment_info[m].lefttrig << " :: " << line->segment_info[m].righttrig << std::endl;

      std::cout << "..line facenum[0]: " << line->segment_info[m].facenum[0]
                << "..line facenum[1]: " << line->segment_info[m].facenum[1]
                << std::endl;
      std::cout << "..facenum[0]: " << segment_info[m].facenum[0]
                << "..facenum[1]: " << segment_info[m].facenum[1]
                << std::endl;

      std::cout << "..left trig size: " << line->segment_info[m].lefttrig_size << " "
                << "..right trig size: " << line->segment_info[m].righttrig_size << std::endl;
#endif
    }
...
#ifdef DEBUGALL
  std::cout << "..inthlint: " << inthlint << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << ".. begin segn: " << segn
	    << "..left trig: " << GetLeftTrig(segn)
	    << "..right trig: " << GetRightTrig(segn)
	    << std::endl;
#endif

  for(int m=0; m<geometry->material_size; m++)
    {
      if(segment_info[m].lefttrig != -1 && segment_info[m].righttrig != -1)
	{
#ifdef DEBUGALL
	  std::cout << "..mat: " << m << std::endl;
#endif

	  line->segment_info[m].lefttriangles[line->segment_info[m].lefttrig_size] = segment_info[m].lefttriangles[segn];
	  line->segment_info[m].righttriangles[line->segment_info[m].righttrig_size] = segment_info[m].righttriangles[segn];

#ifdef DEBUGALL
	  std::cout << "..begin lefttrig at pos " << line->segment_info[m].lefttrig_size << " : "
		    << line->segment_info[m].lefttriangles[line->segment_info[m].lefttrig_size] << std::endl;
	  std::cout << "..begin righttrig at pos " << line->segment_info[m].righttrig_size << " : "
		    << line->segment_info[m].righttriangles[line->segment_info[m].righttrig_size] << std::endl;
#endif
  ...
  for (i = 1; i <= inthlint; i++)
...
#ifdef DEBUGALL
      std::cout << ".. in for .. inthlint: " << inthlint << std::endl;
#endif
      ...
      if (i != inthlint)
	{
	  ...
	  // [FS] added for multiple segments
	  //
	  for(int m=0; m<geometry->material_size; m++)
	    {
		if(segment_info[m].lefttrig != -1 && segment_info[m].righttrig != -1)
...
#ifdef DEBUGALL
		  std::cout << "..middle lefttrig at pos " << line->segment_info[m].lefttrig_size << " : "
			    << line->segment_info[m].lefttriangles[line->segment_info[m].lefttrig_size] << std::endl;
		  std::cout << "..middle righttrig at pos " << line->segment_info[m].righttrig_size << " : "
			    << line->segment_info[m].righttriangles[line->segment_info[m].righttrig_size] << std::endl;
#endif
...
#ifdef DEBUGALL
	  std::cout << "..end lefttrig at pos " << line->segment_info[m].lefttrig_size << " : "
		    << line->segment_info[m].lefttriangles[line->segment_info[m].lefttrig_size] << std::endl;
	  std::cout << "..end righttrig at pos " << line->segment_info[m].righttrig_size << " : "
		    << line->segment_info[m].righttriangles[line->segment_info[m].righttrig_size] << std::endl;
#endif
  ...
  */
#ifdef DEBUGALL
  std::cout << "..EXIT STLLine::Mesh " << std::endl;
#endif
  return line;
}
}
