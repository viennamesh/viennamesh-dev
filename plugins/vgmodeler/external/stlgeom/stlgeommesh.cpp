  ...
  if (checkchart)
      ...
      if (trigs)
	{
	  ...
	  while (*htrigs)
		...
		{
#ifdef DEBUGALL
                   std::cout << ".. in foundinchart 1 .. " << std::endl;
#endif
		  foundinchart = 1;
		  break;
		}
      ...
      else
	{
	  ...
	  for (i = 1; i <= trigsinbox.Size(); i++)
		...
		{
#ifdef DEBUGALL
                   std::cout << ".. in foundinchart 2 .. " << std::endl;
#endif
		  foundinchart = 1;
		  break;
		}
...
{
#ifdef DEBUGALL
  std::cout << ".. ENTER RestrictLocalH .. " << std::endl;
#endif
  ...
  // [FS] .. try this - uncomment SetGlobalH and use the passed on LFS
  //                    then the LFS is used for the whole mesh
  //                    for setting the boundary LFS
  mesh.SetGlobalH(gh);
  ...
  if (stlparam.resthsurfcurvenable)
    {
#ifdef DEBUGALL
      std::cout << "Restrict H due to surface curvature" << std::endl;
#endif
  ...
  if (stlparam.resthcloseedgeenable)
    {
#ifdef DEBUGALL
      std::cout << "Restrict H due to close edges" << std::endl;
#endif
...
#ifdef DEBUGALL
      std::cout << "build search tree... disttohfact: " << disttohfact << std::endl;
#endif
...
#ifdef DEBUGALL
      std::cout << ".. GetNLines: " << GetNLines() << std::endl;
#endif
...
#ifdef DEBUGALL
		      std::cout << " .. h2: " << h2 << " :: dist: " << dist << std::endl;
#endif

		      if (dist > 1E-12)
			{
// 			  std::cout << "..in if dist > 1E-12" << std::endl;
...
#ifdef DEBUGALL
      std::cout << "done\n Restricted h in " << rc << " points due to near edges!" << std::endl;
#endif
      PopStatus();
    }
...
#ifdef DEBUGALL
      std::cout << "edge-angle min local h=" << mincalch << "\nedge-angle max local h=" << maxcalch << std::endl;
#endif
      PopStatus();
    }
  ...
  if (stlparam.resthchartdistenable)
    {
#ifdef DEBUGALL
      std::cout << "Restrict H due to outer chart distance" << std::endl;
#endif
...
#ifdef DEBUGALL
      std::cout << "Restrict H due to line-length" << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << ".. EXIT RestrictLocalH .. " << std::endl;
#endif
}
...
{
#ifdef DEBUGALL
  std::cout << "..ENTER RestrictHChartDistOneChart" << std::endl;
#endif
...
#ifdef DEBUGALL
       std::cout << "after GetChartTrig" << std::endl;
#endif

      tt = GetTriangle(t);

#ifdef DEBUGALL
       std::cout << "after GetTriangle" << std::endl;
#endif

      for (k = 1; k <= 3; k++)
	{
#ifdef DEBUGALL
       std::cout << "in 2. for - before NeighbourTrig" << std::endl;
#endif
	  nt = NeighbourTrig(t,k);

#ifdef DEBUGALL
	  std::cout << "..RHCDOC: nt: " << nt << std::endl;
#endif

	  if (GetChartNr(nt) != i)
	    {
#ifdef DEBUGALL
	      std::cout << "..RHCDOC: cp 11 " << std::endl;
#endif

	      tt.GetNeighbourPoints(GetTriangle(nt),np1,np2);

#ifdef DEBUGALL
	      std::cout << "..RHCDOC: cp 111 np1: " << np1 << " :: np2: " << np2 << std::endl;
#endif

	      if (!IsEdge(np1,np2) && !GetSpiralPoint(np1) && !GetSpiralPoint(np2))
		{
#ifdef DEBUGALL
		  std::cout << "..RHCDOC: cp 12 " << std::endl;
#endif
...
#ifdef DEBUGALL
		  std::cout << "..RHCDOC: cp 13 " << std::endl;
#endif
...
#ifdef DEBUGALL
		  std::cout << "..RHCDOC: cp 14 " << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << "..RHCDOC: checkpoint 2 " << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << "..RHCDOC: checkpoint 3 " << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << "..RHCDOC: checkpoint 4 " << std::endl;
#endif
...
#ifdef DEBUGALL
  std::cout << "..EXIT RestrictHChartDistOneChart" << std::endl;
#endif
}

////# define DEBUGALL
//void * STLMeshingDummy (void *)
int STLMeshingDummy (STLGeometry* stlgeometry, Mesh*& mesh, viennagrid::mesh_t const & vgrid_mesh, int perfstepsstart, int perfstepsend)
{
      ...
      perfstepsend   >= MESHCONST_MESHSURFACE)
  {
    ...
    // --------------------------------------------------------------------
    // --------------------------------------------------------------------
    int retval = STLSurfaceMeshing (*stlgeometry, *mesh, vgrid_mesh); // at meststlsurface.cpp
    // --------------------------------------------------------------------
    // --------------------------------------------------------------------
    // --------------------------------------------------------------------
