...
{
  // [FS][INFO] triangles must have the same orientation
  //
  int i, j;
  for(i = 0; i <= 2; i++)
    {
...
//   INDEX_2_HASHTABLE<int> * edges = new INDEX_2_HASHTABLE<int> (20);

//   // [FS] build the edges from the first triangle
//   //
//   for(i=1; i<=3 ; i++)
//     {
...
// 	  int pi1 = PNum(i);
// 	  int pi2 = PNum(j);
// 	  std::cout << " p1: " << pi1 << " p2: " << pi2 << std::endl;
...
//   // [FS] also build edges and test against edges from other trig
//   //
//   for(i=1; i<=3 ; i++)
//     {
//       std::cout << " i: " << i << std::endl;
...
{
  // [FS][INFO] triangles do not have the same orientation
  //
  int i, j;
  for(i = 0; i <= 2; i++)
    {
...
// //   std::cout << "[FS] .. in GetNeighbourPoints " << std::endl;

//   for(i = 1; i <= 3; i++)
//     {
//       for(j = 1; j <= 3; j++)
// 	{
//  	  std::cout << ".. in GetNeighbourPoints .. i: " << i << " .. j: " << j << std::endl;
// 	  std::cout << ".. t.PNumMod(i+1): " << t.PNumMod(i+1)
// 		    << " :: PNumMod(j): " << PNumMod(j)
// 		    << " :: t.PNumMod(i): " << t.PNumMod(i)
// 		    << " :: PNumMod(j+1): " << PNumMod(j+1)
// 		    << std::endl;

// 	  if (t.PNumMod(i+1) == PNumMod(j) && t.PNumMod(i) == PNumMod(j+1))
// 	    {
...
// //   std::cout << "[FS] .. leaving GetNeighbourPoints " << std::endl;
// }
  ...
  for(int i=0; i<10; i++)
    {
      ...
      meta_info.status = ED_UNDEFINED;
      segment_info[i] = meta_info;
    }
}
...
ostream& operator<<(ostream& os, const STLTopEdge& edge)
{
  os << "[";
  os << edge[0] << ",";
  os << edge[1] << "] ";
