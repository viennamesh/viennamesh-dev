...
public:
  ...
  //
  int material[2];
  segment_info_t segment_info[100];
...
class STLTopEdge
{
  ...
  // [FS][TODO] .. extend this information for every material => std::map<int, std::vector<int> >
  //               first index is the material number, second are the triangle indices
  //               do this for trigs, cosangle
  //               maybe also access functions have to be adapted
  //
  int trigs[2];
  double cosangle;
...
public:
   ...
   // [FS][HACK]
  int material;
  meta_info_t segment_info[100];

  // [INFO] segment , meta info for this segment
  //
//   std::map<int, meta_info_t> segment_info;
...
protected:
  ...
  INDEX_3_HASHTABLE<int> * ht_topedges2;
...
public:
  ...
  static STLGeometry * Load (const char* filename);
  ...
  virtual void InitSTLGeometry (viennagrid::mesh_t const & mesh);
