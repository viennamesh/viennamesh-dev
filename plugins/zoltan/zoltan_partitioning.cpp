...
{
		zoltan_partitioning::zoltan_partitioning()	{}
		std::string zoltan_partitioning::name() {return "zoltan_partitioning";}
    ...
    static const char *global_fname="examples/data/objects.txt"; // File containing objects to be partitioned.
    ...
    bool zoltan_partitioning::run(viennamesh::algorithm_handle &)
		{
      std::cout << name() << std::endl;

      data_handle<int> argchars = get_input<int>("argc");
      std::cerr << "ARGC: " << argchars() << std::endl;

      char** argv;
      std::string argument= "test";
      argv[0] = new char[argument.size()+1];
      std::copy(argument.begin(), argument.end(), argv[0]);
      argv[0][argument.size()] = '\0';
      ...
      //assume we have a vector containing several pragmatic data structures representing partitions
      size_t vec_size = 64;
      std::vector<int> partitions(vec_size);
