    ...
    public:
      ...
      bool generic_run( std::map<int, int> const & segment_mapping )
      {
        ...
        if (mesh)
        {
          ...
          map_segments( mesh().mesh, mesh().segmentation, output_mesh().mesh, output_mesh().segmentation, segment_mapping );
