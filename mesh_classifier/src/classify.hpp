...
#include "gml/geometry/functors/cell_quality.hpp"
#include "gml/geometry/functors/metric_quantity.hpp"
#include "gml/algorithms/detail/classify_3d.hpp"
#include "gml/algorithms/detail/keys.hpp"
//
// *** GSSE includes
//
#include <gsse/datastructure/domain.hpp>
#include <gsse/algorithm/io.hpp>
#include <gsse/util/latex_histogram.hpp>
//
// *** SYSTEM includes
//
#include <set>
...
struct classify < 2 >
{
   ...
   static void
   compute( Domain& domain, std::string prefix = "" )
   {
   #ifdef DEBUG_CLASSIFY
      std::cout << "classifying a gau2 domain " << std::endl;
   #endif           
      ...
      //
      // declare and define the gml generic functors
      //
      typedef gsse::boundary< 2, 0 >                                          TopologyOperationQuality;
      typedef gml::property_data< TopologyOperationQuality, CellContainer >   PropertyDataQuality;
      typedef typename gml::cell_quality< PropertyDataQuality >::type         Quality;
      Quality qual;

   #ifdef DEBUG_CLASSIFY
      std::cout << "using quality: "; gml::dumptype< Quality >();      
   #endif           
      // ----
      typedef gsse::boundary< 2, 1 >                                          TopologyOperationMetric;
      typedef gml::property_data< TopologyOperationMetric, CellContainer >    PropertyDataMetric;
      typedef typename gml::metric_quantity< PropertyDataMetric >::type       CellLengths;
      // [JW]TODO we need to derive it from the property ?! 
      // however, right now, there is no coupling of the types!
      typedef typename gml::result_of::metric_quantity< double >::type        MetricQuantityResult;                     
      CellLengths cell_lengths;      
      // ----
      typedef gsse::boundary< 1, 0 >                                          TopologyOperationEdge;
      typedef gml::property_data< TopologyOperationEdge, CellContainer >      PropertyDataEdge;
      typedef typename gml::metric_quantity< PropertyDataEdge >::type         Edgeength;
      Edgeength edge_length;      

      const double PI  = 3.14159265;
      const double PI2 = 6.2831853;
      
      using namespace gml::detail::keys;
      ...
      for( size_t si = 0; si < gsse::size( segments_topology ); si++ )
      {
         ...
         for( size_t ci = 0; ci < gsse::size( cell_cont ); ci++ )
         {
            ...
            gml::for_each( angles, phoenix::push_back( phoenix::ref( angles_out ), _1 ) );               
   ...
   
      {
         ...
         if(prefix == "")
            file = "latex.txt";
         else file = prefix+"_latex.txt";
         stream.open(file.c_str());   
         gsse::writeLatex(histogram, true, true, stream);
         //gml::writeLatex<2>::run(histogram, stream);
         stream.close();         
      }
      {
         std::ofstream  stream; 
...
struct classify < 3 >
{
   ...
   {
   #ifdef DEBUG_CLASSIFY
      std::cout << "classifying a gau3 domain " << std::endl;
   #endif        
      ...
      Histogram histogram;
      typedef gml::external::SimplexClassification<double> SimplexCl;   
      ...
      for( size_t si = 0; si < gsse::size( segments_topology ); si++ )
      {
         ...
         for( size_t ci = 0; ci < gsse::size( cell_quality_quan_cont ); ci++ )
         {
            gsse::at(0)( gsse::at( ci )( cell_quality_quan_cont ) ) = 0.0;
            gsse::at(0)( gsse::at( ci )( cell_round_quan_cont ) )   = 0.0;
      ...
      for( size_t si = 0; si < gsse::size( segments_topology ); si++ )
      {
         ...
         for( size_t ci = 0; ci < gsse::size( cell_cont ); ci++ )
         {
            ...
            gml::for_each( simplexc1.DiAngle, phoenix::push_back( phoenix::ref( angles ), _1 ) );
      ...
      }         
      {
         ...
         stream.open(file.c_str());   
         gsse::writeLatex(histogram, true, true, stream);
         //gml::writeLatex<3>::run(histogram, stream);
         stream.close();         
      }
      {
         std::ofstream  stream; 
...
void classify( DomainT& domain, std::string prefix = "" )
{
   gml::detail::classify<
      gsse::result_of::property_DIMT<
         gsse::property_domain<DomainT>
      >::value 
   >::compute(domain, prefix);
}
//
// ==================================================================
//
