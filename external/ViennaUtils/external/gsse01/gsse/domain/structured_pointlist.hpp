...
#ifndef GSSE_STRUCTURED_POINTLIST_HH_ID
#define GSSE_STRUCTURED_POINTLIST_HH_ID
...
class structured_pointlist
{
public:

   typedef structured_pointlist<TopologyT, PointT>                      self;
...
public:
   structured_pointlist() {}

   structured_pointlist(long dimension)
   {
      dimension_ticks.resize(dimension, std::vector<numeric_t>()) ;
   }

   structured_pointlist(const std::vector<std::vector<numeric_t> >& list_dim )
   {
