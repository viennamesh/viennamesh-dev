...
(ARRAY<class SpecialPoint> & points)
{
  ...
  for (i = 1; i <= points.Size(); i++)
      ...
      for (j = 1; j <= points.Size(); j++)
	{
      ...
      std::cout << "Identify Periodic special points: pi = " 
...
GetIdentifiedPoint (class Mesh & mesh,  int pi)
{
  ...
  /* 
  std::cout << "Identify points(periodic), nr = " << nr << ": " << mesh.Point(pi)
  ...
  for (i = 1; i <= mesh.GetNP(); i++)
      ...
      if (s1->PointOnSurface (p))
	{
	  ...
	  for (j = 1; j <= mesh.GetNP(); j++)
		...
		/*
		std::cout << "Identify points(periodic:), nr = " << nr << ": "
...
void PeriodicIdentification :: IdentifyFaces (class Mesh & mesh)
{
  ...
  for (i = 1; i <= mesh.GetNFD(); i++)
	...
	//	std::cout << "check faces " << i << " and " << j << endl;
		...
		for (l = 1; l <= mesh.GetNSeg(); l++)
		  {
		    ...
		    //		    std::cout << "seg1 = " << seg1.p1 << "-" << seg1.p2 << ", seg2 = " << seg2.p1 << "-" << seg2.p2;
	...
	if (idok)
	  {
	    // std::cout << "Identify faces " << i << " and " << j << endl;
	    INDEX_2 fpair(i,j);
	    fpair.Sort();
		      ...
		      Mesh & mesh, const Surface * surf)
{
	  ...
	  if (identfaces.Used (fpair))
	    {
	      ...
	      // copy element
	      Element2d newel(sel.GetNP());
	      newel.SetIndex (facei);
	      for (k = 1; k <= sel.GetNP(); k++)
		{
		  newel.PNum(k) = 
	      ...
	      if (nsurf * nt < 0)
		Swap (newel.PNum(1), newel.PNum(3));
  ...
  if (found)
    {
      std::cout << " copy face " << facei << " from face " << fother;
      segs.SetSize(0);
    }
}
...
(ARRAY<class SpecialPoint> & points)
{
  ...
  for (i = 1; i <= points.Size(); i++)
      ...
      for (j = 1; j <= points.Size(); j++)
	{
      ...
      std::cout << "Identify close surfaces special points: pi = " 
		 << points.Get(i).p << ", vi = " << points.Get(i).v 
		 << " pj = " << points.Get(bestj).p 
		 << ", vj = " << points.Get(bestj).v 
		 << " bestval = " << bestval << endl;
...
Identifyable (const SpecialPoint & sp1, const SpecialPoint & sp2,
	      ...
	      const TABLE<int> & specpoint2surface) const
{
  //std::cout << "identcheck: " << sp1.p << "; " << sp2.p << endl;
  ...
  if (!dom_surf_valid)
      ...
      if (domain)
	{
	  ...
	  //std::cout << "surfs of identification " << nr << ": " << endl << hsurf << endl;
	}
      else
	{
  ...
  if (!s1->PointOnSurface (sp1.p))
    {
      //std::cout << "failed at pos2" << endl;
      return 0;
    }

//   std::cout << "sp1 " << sp1.p << " sp2 " << sp2.p << endl
...
// 	     << "specpoint2solid[sp2.nr] " << specpoint2solid[sp2.nr] << endl;
  ...
  if ( fabs(n1 * sp1.v) > eps_n)
    {
      //std::cout << "failed at pos3" << endl;
      return 0;
    }

  if (!s2->PointOnSurface(sp2.p))
    {
      //std::cout << "failed at pos4" << endl;
      return 0;
    }
...
GetIdentifiedPoint (class Mesh & mesh,  int pi)
{
  ...
  ARRAY<INDEX,PointIndex::BASE> identmap(mesh.GetNP());
  ...
  if (s1->PointOnSurface (p))
    ...
    {
      std::cout  << "GetIdenfifiedPoint: Not possible" << endl;
      std::cout << "p = " << p << endl;
      std::cout << "surf1: " << (*s1) << endl
		 << "surf2: " << (*s2) << endl;
  ...
  //std::cout << "projecting " << p << " to " << hp << endl;
  ...
  if (snew == s2)
    {
      mesh.GetIdentifications().Add (pi, newpi, nr);
      //std::cout << "add identification(1) " << pi << " - " << newpi << ", " << nr << endl;
    }
  else
    {
      mesh.GetIdentifications().Add (newpi, pi, nr);
      //std::cout << "add identification(2) " << newpi << " - " << pi << ", " << nr << endl;
    }
	   

  /*
  std::cout << "Identify points(closesurface), nr = " << nr << ": " << mesh.Point(pi)
	     << " and " << mesh.Point(newpi) 
	     << ((snew == s2) ? "" : " inverse")
	     << endl;
  */
  ...
  for (int i1 = 1; i1 <= np; i1++)
    {
      ...
      //      std::cout << "p1 = " << i1 << " = " << p1 << endl;
      if (domain && !domain->GetSolid()->IsIn (p1))
	continue;
      
      //if(domain) std::cout << "p1 is in " << domain->GetSolid()->Name() << endl;
      ...
      if (s1->PointOnSurface (p1))
	{
	  ...
	  //std::cout << " surfs of p1 = " << endl << surfs_of_p1 << endl;
	  ...
	  if (candi2)
	    {
	      //std::cout << "identify points " << p1 << " - " << mesh.Point(candi2) << endl;

	      /*
	      std::cout << "Add Identification from CSI2, nr = " << nr << ", p1 = " 
			 << i1 << " = " 
			 << mesh[PointIndex(i1)] << ", p2 = " << candi2 << " = " 
			 << mesh[PointIndex(candi2)] << endl;
	      */
	      mesh.GetIdentifications().Add (i1, candi2, nr);
	      //std::cout << "add identification " << i1 << " - " << candi2 << ", " << nr << endl;
	    }
	}
    }
}
...
void CloseSurfaceIdentification :: IdentifyFaces (class Mesh & mesh)
{
  ...
  //std::cout << "identify faces, nr = " << nr << endl;
  ...
  for (int i = 1; i <= mesh.GetNFD(); i++)
      ...
      for (int j = 1; j <= mesh.GetNFD(); j++)
	{
	  ...
	  if (idok)
	    {
	      //std::cout << "Identification " << nr << ", identify faces " << i << " and " << j << endl;
	      INDEX_2 fpair(i,j);
	      fpair.Sort();
	      identfaces.Set (fpair, 1);
	    }
		      ...
		      Mesh & mesh, const Surface * surf)
{
  ...
  ARRAY<INDEX,PointIndex::BASE>  identmap;
  identmap = 0;
  ...
  //std::cout << "identification nr = " << nr << endl;
  //std::cout << "surf = " << (*surf) << endl;
  //std::cout << "domain = " << domain->GetSolid()->Name() << endl;
  //std::cout << "segs = " << endl << segs << endl;
  //std::cout << "identmap = " << endl << identmap << endl;
	...
	for (int i2 = 0; i2 < i1; i2++)
	  {
	    ...
	    //std::cout << "checking " << s1 << " and " << s2 << " for ident." << endl;
...
//  		std::cout << "(id nr "<< nr <<") add rect element: "
		       ...
		       Mesh & mesh, const Surface * surf)
{
  ...
  //  std::cout << "copy trig face, identnr = " << nr << endl;
  //  std::cout << "identfaces = " << endl << identfaces << endl;
  ...
  for (int i = 1; i <= identfaces.GetNBags(); i++)
	...
	std::cout << "identface = " << i2 << endl;
	std::cout << "face " << i2.I1() << " = " << mesh.GetFaceDescriptor(i2.I1()) << endl;
	std::cout << "face " << i2.I2() << " = " << mesh.GetFaceDescriptor(i2.I2()) << endl;
      }
  */

  if (foundid)
    {
      //	  std::cout << "surfaces found" << endl;
      // copy surface
...
void CloseEdgesIdentification :: IdentifyPoints (Mesh & mesh)
{
  ...
  for (i1 = 1; i1 <= np; i1++)
	...
	if (fabs (n * t) < 0.5)
	  {
	    std::cout << "close edges identify points " << p1 << " - " << p2 << endl;
  ...
  for (i1 = 1; i1 <= segs.Size(); i1++)
	...
	if (mesh.GetIdentifications().Get (s1.p1, s2.p2) &&
	    mesh.GetIdentifications().Get (s1.p2, s2.p1))
	  {
	    ...
	    //std::cout << "n = " << n << " ns = " << ns << endl;
	    if (n * ns < 0)
	      {
		//std::cout << "Swap the quad" << endl;
		Swap (el.PNum(1), el.PNum(2));
		Swap (el.PNum(3), el.PNum(4));
	      }
