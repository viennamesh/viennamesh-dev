...
extern double minwithoutother;
 ...
 double CalcElementBadness (const ARRAY<Point3d> & points,
				  const Element & elem)
{
  ...
  //  std::cout << "l^3/vol = " << (l*l*l / vol) << endl;
...
//   NgProfiler::RegionTimer reg3(99);
  ...
  if (testmode)
    std::cout << "cnt = " << cnt << " class = " << tolerance << endl;
...
#ifdef DEBUG      
	std::cout << "Rule " << ri << " = " << rule->Name() << endl;
#endif
	  ...
	  if (nfok <= rule->GetNOldF())
	      ...
	      if (ok)
		{
		  ...
		  for (j = 1; j <= rule->GetNP (nfok); j++)
			...
			std::cout << "change face1 point, mark1" << endl;
...
// 	      NgProfiler::RegionTimer regfb(301);
	      ...
	      while (npok >= 1)
		{
		  ...
		  if (npok <= rule->GetNOldP())
			  ...
			  if (ok)
			    {
			      ...
			      if (npok <= 3)
				std::cout << "set face1 point, mark3" << endl;
			    ...
			    
			    {
			      ...
			      if (npok <= 3)
				std::cout << "set face1 point, mark4" << endl;
		    ...
		    {
// 		      NgProfiler::RegionTimer regfa(302);		      

		      // all points are mapped
		      
		      if (loktestmode)
			{
			  std::cout << "Mapping found!!: Rule " << rule->Name() << endl;
			  for (i = 1; i <= pmap.Size(); i++)
			    std::cout << pmap.Get(i) << " ";
			  std::cout << endl;
			  sprintf (problems.Elem(ri), "mapping found");
			  std::cout << rule->GetNP(1) << " = " << lfaces.Get(1).GetNP() << endl;
			}
		      ...
		      for (i = 1; i <= rule->GetNE(); i++)
			{
			  ...
			  if (el.GetType() == PYRAMID) 
			    { 
			      if (loktestmode)
				std::cout << "map pyramid, rule = " << rule->Name() << endl;
			      for (j = 1; j <= 2; j++)
				{
				  INDEX_2 in2;
				  if (j == 1)
		      ...
		      if (loktestmode)
			{
			  ...
			  std::cout << "Freezone: " << endl;
			  for (i = 1; i <= fz.Size(); i++)
			    std::cout << fz.Get(i) << endl;
			}
		      ...
		      for (i = 1; i <= lfaces.Size() && ok; i++)
			{
			  ...
			  if (!fused.Get(i))
			      ...
			      if (triin == 1)
				{
				  ...
				  if (loktestmode)
				    {
				      std::cout << "El with " << lfaces.Get(i).GetNP() << " points in freezone: "
						 << lfaces.Get(i).PNum(1) << " - " 
						 << lfaces.Get(i).PNum(2) << " - "
						 << lfaces.Get(i).PNum(3) << " - "
						 << lfaces.Get(i).PNum(4) << endl;
				      for (int lj = 1; lj <= lfaces.Get(i).GetNP(); lj++)
					std::cout << lpoints.Get(lfaces.Get(i).PNum(lj)) << " ";
...
// 				    {
				      ...
				      if (lfacei.GetNP() == 3)
					{
					  std::cout << "Triangle in freezone: "
						     ...
						     << lpoints.Get(lfacei.PNum(3)) 
						     << endl;
					  std::cout << "lpi = " << lpi.Get(1) << ", " 
						     << lpi.Get(2) << ", " << lpi.Get(3) << endl;
					}
				      else
					  std::cout << "Quad in freezone: "
						     << lfacei.PNum(1) << " - " 
						     << lfacei.PNum(2) << " - "
						     << lfacei.PNum(3) << " - "
						     << lfacei.PNum(4) 
...
// 				    }	
 ...
 // 						std::cout << "found from other side: " 
//  							   << rule->Name() 
...
// 			    }
				  ...
				  if (loktestmode)
				    {
				      ...
				      std::cout << "Orientation wrong" << endl;
				    }
				  ok = 0;
				}
			    }
			  ...
			  for (i = rule->GetNOldP() + 1; i <= rule->GetNP() && ok; i++)
				...
				if (loktestmode)
				  {
				    std::cout << "Newpoint " << lpoints.Get(pmap.Get(i))
			  ...
			  if (ok && teterr < tolerance)
			    {
			      canuse.Elem(ri) ++;
			      /*
			      std::cout << "can use rule " << rule->Name() 
					 << ", err = " << teterr << endl;
			      for (i = 1; i <= pmap.Size(); i++)
				std::cout << pmap.Get(i) << " ";
			      std::cout << endl;
