...
{
//    int timer = NgProfiler::CreateTimer ("topology");
//   NgProfiler::RegionTimer reg (timer);
  ...
  PrintMessage (3, "Update mesh topology");
   ...
   std::cout << " UPDATE MESH TOPOLOGY " << endl; 
   std::cout << "ne   = " << ne << endl;
   std::cout << "nse  = " << nse << endl;
   std::cout << "nseg = " << nseg << endl;
   std::cout << "np   = " << np << endl;
   std::cout << "nv   = " << nv << endl;
  ...
  if (buildedges)
    {
//        int timer1 = NgProfiler::CreateTimer ("topology::buildedges");
//       NgProfiler::RegionTimer reg (timer1);
      ...
      for (i = 1; i <= missing.Size(); i++)
	{
      ...
      /*
	std::cout << "edge table:" << endl;
	std::cout << "edge2vert:" << endl;
	for (i = 1; i <= edge2vert.Size(); i++)
	std::cout << "edge " << i << ", v1,2 = " << edge2vert.Elem(i)[0] << ", " << edge2vert.Elem(i)[1] << endl;
	std::cout << "surfedges:" << endl;
  ...
  if (buildfaces) //  && mesh.GetDimension() == 3)
    {
//        int timer2 = NgProfiler::CreateTimer ("topology::buildfaces");
//       NgProfiler::RegionTimer reg (timer2);
      ...
      if (ne)
	{
	  ...
	  for (i = 0; i < nfa; i++)
	    {
	      /*
	      std::cout << "face " << i << " has " << int(face_els[i]) << " els, " 
	      ...
	      if (face_els[i] + face_surfels[i] == 1)
		{
		  ...
		  std::cout << "exchange face : " << i << endl;
		  std::cout << "points = " << face2vert[i] << endl;
		  std::cout << "global points = ";
		  for ( int j = 0; j < 3; j++ )
// 		    std::cout << face2vert[i].I(j+1) << " -> " 
// 			       << paralleltop.GetLoc2Glob_Vert( face2vert[i].I(j+1) ) << ",  ";
		  ...
		  for (int j = 0; j < 4; j++)
		    {
		      paralleltop.SetExchangePoint(face2vert[i].I(j+1));
		      std::cout << "set exvert " << face2vert[i].I(j+1) << endl;
		    }
		  ARRAY<int> faceedges;
		  ...
		  for ( int j = 0; j < faceedges.Size(); j++)
		    {
		      ...
		      std::cout << faceedges[j] << "  v " << v1 << " " << v2 << endl;
		    }
		  std::cout << "pos = ";
		  for (int j = 0; j < 4; j++)
		    if (face2vert[i].I(j+1) >= 1)
		      std::cout << mesh[(PointIndex)face2vert[i].I(j+1)] << " ";
		  std::cout << endl;
#else
		  std::cout << "illegal face : " << i << endl;
		  std::cout << "points = " << face2vert[i] << endl;
		  std::cout << "pos = ";
		  for (int j = 0; j < 4; j++)
		    if (face2vert[i].I(j+1) >= 1)
		  ...
		  for (int k = 0; k < vertels.Size(); k++)
		    {
		      int faces[10], orient[10];
		      int nf = GetElementFaces (vertels[k], faces, orient);
		      for (int l = 0; l < nf; l++)
			if (faces[l] == i)
			  {
			    std::cout << "is face of element " << vertels[k] << endl;
			    
			    if (mesh.coarsemesh && mesh.hpelements->Size() == mesh.GetNE() )
			      {
				const HPRefElement & hpref_el =
				  (*mesh.hpelements) [ mesh.VolumeElement (vertels[k]).hp_elnr];
				std::cout << "coarse eleme = " << hpref_el.coarse_elnr << endl;
			      }
	...
	{
 	  paralleltop.Update();
	  paralleltop.UpdateExchangePoints();      
          paralleltop.UpdateExchangeFaces();
	  paralleltop.UpdateExchangeEdges();
	}
      paralleltop.Print();
#endif
...
for (i = 1; i <= ne; i++)
    ...
    std::cout << "PNums " << endl; 
    for( int l=1;l<=8;l++) std::cout << mesh.VolumeElement(i).PNum(l) << "\t"; 
    std::cout << endl; 
    std::cout << "edges: " << endl;
    for (j = 0; j < 9; j++)
    std::cout << edges.Elem(i)[j] << " ";
    std::cout << "faces: " << endl;
    for (j = 0; j < 6; j++)m
    std::cout << faces.Elem(i)[j] << " ";
    }

    for (i = 1; i <= nse; i++)
    {
    std::cout << "SElement " << i << endl;
    std::cout << "PNums " << endl; 
    for( int l=1;l<=4;l++) std::cout << mesh.SurfaceElement(i).PNum(l) << "\t"; 
    std::cout << endl; 
    }
  */
...
void MeshTopology :: GetFaceEdges (int fnr, ARRAY<int> & edges) const
{
  ...
  for (int i = 0; i < els.Size(); i++)
      ...
      if(fa>=0)
	{
	  ...
	  // std::cout << " Face " << fnr << endl; 
	  // std::cout << " GetFaceEdges " << edges << endl;
...
int MeshTopology :: GetVerticesEdge ( const int v1, const int v2 ) const
{
...
void MeshTopology :: GetSegmentVolumeElements ( const int segnr, ARRAY<int> & volels ) const
{
  int v1, v2;
  ...
  for ( int eli1=0; eli1<volels1.Size(); eli1++)
    if ( volels2.Contains(volels1[eli1]) )
      volels.Append ( volels1[eli1] );
