		 ...
		 INDEX_2_HASHTABLE<int> & surf_edges, ARRAY<int, PointIndex::BASE> & facepoint)
{
	...
	if (debug)
	  {
	    cout << "debug" << endl;
	    std::cout  << "debug" << endl;
	    std::cout << "ep = " << ep1 << ep2 << ep3 << ep4 << endl;
	    std::cout << "cp = " << cp1 << cp2 << cp3 << cp4 << endl;
	    std::cout << "edge = " << isedge1 << isedge2 << isedge3 << isedge4 << isedge5 << isedge6 << endl;
	  }
	...
	for (int l = 0; l < 4; l++)
	  {
	    ...
	    switch (l)
	      {
	      case 0: i3 = INDEX_3 (el.pnums[k], el.pnums[pi3], el.pnums[pi4]); break;
	      case 1: i3 = INDEX_3 (el.pnums[j], el.pnums[pi3], el.pnums[pi4]); break;
	      case 2: i3 = INDEX_3 (el.pnums[j], el.pnums[k], el.pnums[pi4]); break;
	      case 3: i3 = INDEX_3 (el.pnums[j], el.pnums[k], el.pnums[pi3]); break;
	      }
	    i3.Sort();
	    if (faces.Used (i3))
	      {
	...
	for (int l = 0; l < 6; l++)
	  {
	    ...
	    switch (l)
	      {
	      case 0: i2 = INDEX_2 (el.pnums[j], el[k]); break;
	      case 1: i2 = INDEX_2 (el.pnums[j], el.pnums[pi3]); break;
	      case 2: i2 = INDEX_2 (el.pnums[j], el.pnums[pi4]); break;
	      case 3: i2 = INDEX_2 (el.pnums[k], el.pnums[pi3]); break;
	      case 4: i2 = INDEX_2 (el.pnums[k], el.pnums[pi4]); break;
	      case 5: i2 = INDEX_2 (el.pnums[pi3], el.pnums[pi4]); break;
	      }
	    i2.Sort();
	...
	for (int l = 0; l < 4; l++)
	  {
	    ...
	    switch (l)
	      {
	      case 0: pi = el.pnums[j]; break;
	      case 1: pi = el.pnums[k]; break;
	      case 2: pi = el.pnums[pi3]; break;
	      case 3: pi = el.pnums[pi4]; break;
	      }
	...
	if (type != HP_NONE)
	  {
	    ...
	    for(int k=0;k<4;k++) el.pnums[k] = pnums[k]; 
	    break;
	  }
      }
  ...
  if (type == HP_NONE)
    {
      //     cnt_undef++;
      std::cout << "undefined element" << endl
		 ...
		 INDEX_2_HASHTABLE<int> & surf_edges, ARRAY<int, PointIndex::BASE> & facepoint)
{
  ...
  /*
   std::cout << " Prism with pnums " << endl; 
   for(int j=0;j<6;j++) std::cout << el.pnums[j] << "\t"; 
   std::cout << endl; 
   */
  ...
  /* std::cout << " Classified Prism with pnums " << endl; 
  for(int j=0;j<6;j++) std::cout << el.pnums[j] << "\t"; 
  std::cout << endl; 
  */ 
  return(type); 
...

{
  ...
  // std::cout << " facepoint " << facepoint << endl;  
  ...
  for (int j=0;j<3;j++) 
    {
      ...
      std::cout << " actual rotation of trig points " ;  
      for(int m=0;m<3;m++) 
	{ 
      ...
      if(dim == 3) 
	{
	  ...
	  for(int k=0;k<3;k++)
	    if(!sing_face)
	      { 
		//	std::cout << " fp [" << k << "] = " << facepoint[pnums[k]] << endl;   
		//	std::cout << " fd.DomainIn()" <<  fd.DomainIn() << endl; 
		//	std::cout  << " fd.DomainOut()" <<  fd.DomainOut() << endl; 
	  ...
	  /*	  std::cout << " pointsing NACH FACEPOints ... FALLS EDGEPOINT UMSETZEN" ; 
	  for (int k=0;k<3;k++) std::cout << "\t" << point_sing[p[k]-1] ;
	  std::cout << endl; */
	}
      ...
      if(dim==3)
	{
	  for(int k=0;k<3;k++) 
		...
		{	  
	      ...
	      /*  std::cout << " pointsing NACH edges UND FACEEDGES UMSETZEN ... " ; 
	      for (int k=0;k<3;k++) std::cout << "\t" << point_sing[p[k]-1] ; 
	      std::cout << endl;          
	      */
	    }
      ...
      std::cout << " edgepoint_dom " << edgepoint_dom << endl; 
      */
      if(dim==2)
	{
	  for(int k=0;k<3;k++) 
	...
	type = HP_TRIG_3SINGEDGES; 
     ...
     //std::cout << " run for " <<  j << " gives type " << type << endl; 
  ...
  for (int j = 1; j <= 3; j++)
      ...
      if (dim == 2)
	{
	  ...
	  */
      ...
      // std::cout << "cp = " << cp1 << cp2 << cp3 << ", ep = " << ep1 << ep2 << ep3 << endl;
