...
public:
  ...
  ///
  GeomPoint (double ax, double ay, double aref = 1)
    : Point<D> (ax, ay), refatpoint(aref) { ; }
  GeomPoint (double ax, double ay, double az, double aref)
    : Point<D> (ax, ay, az), refatpoint(aref) { ; }
  GeomPoint (const Point<D> & ap, double aref = 1)
    : Point<D>(ap), refatpoint(aref) { ; }
};
...
std::cout << "cosalpha " << cosalpha << endl;
