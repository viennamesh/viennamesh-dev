...
#ifndef VIENNAMESH_BND_READER_HPP
#define VIENNAMESH_BND_READER_HPP
...
inline std::ostream &operator<<(std::ostream &of, const edge_c &e) 
{ 
   of << "(" << e[0] << "," << e[1] << ")"; return of; 
}
...
private:
    ...
    static const double fac2=1000000;
...
private:
   ...
   std::pair<int,int> check_needle(face_c &face, points_c &points)
   {
      ...
      for (int i=0; i<face.size(); i++)
      {
	      ...
	      double d3=( (points.x[ii]-points.x[jj])*(points.x[ii]-points.x[jj])+ 
	                  (points.y[ii]-points.y[jj])*(points.y[ii]-points.y[jj])+ 
	                  (points.z[ii]-points.z[jj])*(points.z[ii]-points.z[jj]) );
	      //print "d3 ",d3
   ...
   void remove_needles(faces_c &tmpfaces, elements_c &elements, points_c &points)
   {
	   int changed=1;
   ...
   void consistency_check(faces_c &tmpfaces, elements_c &elements, regions_c &regions, polyhedronlist_c &polyhedronlist)
   {
	   edgemap_c edgemap;
	   int i;
	   edge_c edge1,edge2;
   ...
   void build_new_faces1(polyhedronlist_c *_polyhedronlist, std::vector<face_c> &_tmpfaces)
   {
   int i=0;
   foreach (j,_bndfaces)
   {
   ...
   void build_new_faces2(std::vector<face_c> &_tmpfaces)
   {
...
//   }
   ...
   {
      polyhedronlist_c _polyhedronlist;
