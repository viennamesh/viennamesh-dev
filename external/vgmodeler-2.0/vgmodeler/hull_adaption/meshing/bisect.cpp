...
// 	  }
  ...
  ostream & operator<< (ostream & ost, const MarkedQuad & mq)
  {
    ost << "MarkedQuad: " << endl;
...
//		      cout << "tet converted to prism" << endl;
    ...
    if (printmessage_importance>0)
    {
      cout << "copied " << mtets.Size() << " tets, " << mprisms.Size() << " prisms" << endl;
      cout << "       " << mtris.Size() << " trigs, " << mquads.Size() << " quads" << endl;
    }
  }
...
*/
			     ...
			     BisectionOptions & opt)
  {
    ...
    if (mesh.mglevels == 1 || idmaps.Size() > 0)
      BisectTetsCopyMesh(mesh, NULL, opt, idmaps);
			...
			if(st == "elements")
			  {
			    inf >> st;
			    bool isint = true;
			    for(int i=0; isint && i<st.size(); i++)
			      isint = (isdigit(st[i]) != 0);
...
// 				  }
			    ...
			    cout << "marked " << int(float(cnt)/float(mesh.GetNE())*100.) << "\% of the elements" <<endl;
			    if(cnt > 0)
			      marked = 1;
	...
	do
	  {
	      ...
	      if (mtets.Elem(i).marked)
		{
		  ...
		  if (cutedges.Used (edge))
		    ...
		    {
		      Point<3> np = Center (mesh.Point (edge.I1()),
					   mesh.Point (edge.I2()));
	      ...
	      if (mprisms.Elem(i).marked)
		{
		  ...
		  if (cutedges.Used (edge1))
		    ...
		    {
		      Point<3> np = Center (mesh.Point (edge1.I1()),
					    mesh.Point (edge1.I2()));
		  ...
		  if (cutedges.Used (edge2))
		    ...
		    {
		      Point<3> np = Center (mesh.Point (edge2.I1()),
					    mesh.Point (edge2.I2()));
	      ...
	      if (mids.Elem(i).marked)
		{
		  ...
		  for (j = 0; j < edges.Size(); j++)
			...
			{
			  Point<3> np = Center (mesh.Point (edges[j].I1()),
						mesh.Point (edges[j].I2()));
			  newp.Append(mesh.AddPoint(np));
...
// 		    cerr << " to " << mesh.Point (newpi) << endl;
    ...
    */
    ...
    if (mesh.mglevels <= 2)
      {
        if (printmessage_importance>0)
          cout << "RESETTING mlbetweennodes" << endl;
	for (i = 1; i <= np; i++)
	  {
	...
	*/
    ...
    NgProfiler::RegionTimer * regt = new NgProfiler::RegionTimer(reptimer);    
    if(do_repair)
      {
	ARRAY<ElementIndex> bad_elts;
	ARRAY<double> pure_badness(mesh.GetNE());
	...
	Validate(mesh,bad_elts,pure_badness,max_worsening);
	
        if (printmessage_importance>0)
          for(int ii=0; ii<bad_elts.Size(); ii++)
	    cout << "bad element " << bad_elts[ii] << endl;
	if(repaired_once || bad_elts.Size() > 0)
	  {
	    ...
	    RepairBisection(mesh,bad_elts,isnewpoint,*this,
			    pure_badness,max_worsening,idmaps);
	    repaired = true;
	    repaired_once = true;
	    
            if (printmessage_importance>0)
            {
              cout << "Time for Repair: " << double(clock() - t1)/double(CLOCKS_PER_SEC) << endl;
	      cout << "bad elements after repair: " << bad_elts << endl;
            }
