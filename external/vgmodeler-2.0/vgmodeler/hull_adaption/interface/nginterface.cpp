...
void Ng_LoadGeometry (char * filename)
{
  ...
  if ((strcmp (&filename[strlen(filename)-3], "geo") == 0) ||
	   ...
	   (strcmp (&filename[strlen(filename)-3], "Stl") == 0))
    {
      ...
      std::cout << "[FS] .. number of points in geometry: " << stlgeometry->GetNP() << std::endl;
      ...
      std::cout << " .. done reading STL file " << std::endl;
    }
...
#ifdef ACIS
  else if (strcmp (&filename[strlen(filename)-3], "sat") == 0)
    {
                                       ...
                                       double * dxdxi, int sdxdxi)
{
  if (mesh->GetDimension() == 2)
    {
      cout << "multielementtrafo 2d not implemented" << endl;
      /*
      Point<2> xl(xi[0], xi[1]);
      Point<3> xg;
      Mat<3,2> dx;
