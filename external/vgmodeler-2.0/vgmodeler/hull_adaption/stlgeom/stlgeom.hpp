...
#include "stlline.hpp"
  ...
  public:
    ...
    int GetNConfCandEPP(int p, int material) const;
  };
...
//     ARRAY<STLChart*> atlas;
    ...
    //         create an array of atlas
    //
    typedef ARRAY<STLChart*> atlas_t;
    atlas_t atlas;
//     ARRAY<atlas_t> atlases;
  ...
  public:
    ...
    int AddEdge(int p1, int p2m, int mat=-1);

     //[FS] added
     void DelEdge(int i) { edges.DeleteElement(i); }
    ...
    // [FS] .. multiple surface test: uses several atlases
    //
    STLChart& GetChart(int nr) {return *(atlas.Get(nr));};
//     STLChart& GetChart(int nr) {return *(atlases[0].Get(nr));};
...
#include "meshstlsurface.hpp"
  ...
  extern int STLMeshingDummy (STLGeometry* stlgeometry, Mesh*& mesh, viennagrid::result_of::domain<viennagrid::config::triangular_3d>::type & domain,
