...
{
  std::cout << "GetSurfacePoint called for QuadraticSurface" << std::endl;
...
INSOLID_TYPE Ellipsoid :: BoxInSolid (const BoxSphere<3> & box) const
{
  ...
  //  std::cout << "box = " << box << ", val = " << val << ", maxval = " << maxval << endl;
			  ...
			  double h, int & zone) const
{
  ...
  std::cout << "p = " << p << ",  pp = " << pp << ",  pplane = " << pplane << endl;
  */
...
void Cone :: CalcData ()
{
  ...
  // std::cout << "t0vec = " << t0vec << " t0 = " << t0 << endl;
  // std::cout << "t1vec = " << t1vec << " t1 = " << t1 << endl;
  // PrintCoeff std::cout;
}
