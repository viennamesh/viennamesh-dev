...
namespace vgmodeler
{
		     ...
		     ARRAY<MeshPoint> & apoints)
  {
    ...
    std::cout << "Find Special Points" << endl;
    std::cout << "maxsize = " << size << endl;
	...
	CloseSurfaceIdentification * ident =
	  _cast<CloseSurfaceIdentification * >(geometry->identifications[i]);
	...
	for(int j=0; j<points->Size(); j++)
	  {
		...
		std::cout << "added identpoint " << auxpoint << "; proj. of "
			   <<  (*points)[j] << endl;
		break;
	      }
	  }
    ...
    std::cout << "numprim_histogramm = " << endl << numprim_hist << endl;
  }
			...
			int level, bool calccp, bool calcep)
  {
    ...
    /*    
    std::cout << "lev " << level << ", box = " << box << endl;
    std::cout << "calccp = " << calccp << ", calcep = " << calcep << endl;
    std::cout << "locsol = " << *sol << endl;
    */

    if (multithread.terminate)
      {
	std::cout << "boxes = " << boxes << endl;
	std::cout << "boxesinlevel = " << boxesinlevel << endl;
    ...
    bool surecrossp = 0, sureexp = 0;          // sure ...
     ...
     ARRAY<int> locsurf;  // attention: array is 
	...
	*/
			...
			if (Dist (pts[j], box.Center()) < box.Diam()/2)
			  {
				...
				if (AddPoint (pts[j], layer))
				  std::cout << "cross point found, 1: " << pts[j] << endl;
			      }  
			    delete tansol;
			  }
		    }
		...
		for (int k1 = 0; k1 < numprim - 1; k1++)
		  for (int k2 = k1 + 1; k2 < numprim; k2++)
			...
			ComputeCrossPoints (dynamic_cast<const Plane*> (geometry->GetSurface(locsurf[k1])),
					    dynamic_cast<const Plane*> (geometry->GetSurface(locsurf[k2])),
					    qsurf, pts);
			//std::cout << "checking pot. crosspoints: " << pts << endl;
			...
			for (int j = 0; j < pts.Size(); j++)
			  if (Dist (pts[j], box.Center()) < box.Diam()/2)
				...
				{
				  if (AddPoint (pts[j], layer))
				    std::cout << "cross point found, 2: " << pts[j] << endl;
				}  
			      delete tansol;
			    }
		      }
			...
			if (Dist (pts[j], box.Center()) < box.Diam()/2)
			  {
			    ...
			    if (tansol)
			      ...
			      {
				if (AddPoint (pts[j], layer))
				  std::cout << "extremal point found, 1: " << pts[j] << endl;
			      }  
			    delete tansol;
			  }
		    }
	...
	// std::cout << "dec = " << decision << ", surcp = " << surecrossp << endl;
    ...
    // std::cout << "l = " << level << "locsize = " << locsurf.Size() << " possexp = " << possibleexp << "\n";
	...
	/*
	std::cout << "extremal surfs = ";
	for (int k5 = 0; k5 < locsurf.Size(); k5++)
	  std::cout << typeid(*geometry->GetSurface(locsurf[k5])).name() << " ";
	std::cout << "\n";
	*/
	  ...
	  for (int k2 = k1+1; k2 < locsurf.Size(); k2++)
	    {
	      ...
	      /*
	      std::cout << "edgecheck, types = " << typeid(*surf1).name() << ", " << typeid(*surf2).name()
	...
	// std::cout << "l = " << level << " dec/sureexp = " << decision << sureexp << endl;
	...
	if (decision && sureexp)
	  {
	      ...
	      for (int k2 = k1+1; k2 < locsurf.Size(); k2++)
		{
		  ...
		  if (EdgeNewtonConvergence (surf1, surf2, p))
			...
			{
			  std::cout << "extremalpoint (nearly) found:" << pp << endl;

			  if (Dist (pp, box.Center()) < box.Diam()/2 &&
			      sol -> IsIn (pp, 1e-6*size) && !sol->IsStrictIn (pp, 1e-6*size) )
			    {
			      if (AddPoint (pp, layer))
				std::cout << "Extremal point found: " << pp << endl;//"(eps="<<1e-9*size<<")"<< endl;
			    }  
			}            
		    }
		}
    ...
    // std::cout << "l = " << level << " poss cp/ep sure exp = " << possiblecrossp << " " << possibleexp << " " << sureexp << "\n";
					      ...
					      const Surface * f2, Point<3> & p)
  {
	...
	//std::cout << "p " << p << " f1 " << vrs(0) << " f2 " << vrs(1) << " g1 " << g1 << " g2 " << g2 << endl;
						       ...
						       int dir, Point<3> & p)
  {
	...
	/*
	std::cout << "p " << p << " f1 " << rs(0) << " f2 " << rs(1) << endl
		   << " jacobi " << jacobi << endl
		   << " rhs " << rs << endl;
	*/	
    ...
    if (Abs2 (x) > 1e-20)
      {
	std::cout << "Error: extremum Newton not convergent" << endl;
  ...
  /*
    ...
    //  std::cout << "leftside = " << leftside << endl;
  ...
  */
			...
			ARRAY<SpecialPoint> & specpoints)
  {
    ...
    std::cout << "bbox = " << bbox << endl;
    std::cout << "points = " << apoints << endl;
	...
	for (int i = 0; i < apoints.Size(); i++)
	  {
...
#ifdef DEVELOP
	    std::cout << "surfs = " << surfind << endl;
	    std::cout << "rep_surfs = " << rep_surfind << endl;
#endif
	    ...
	    */
	      ...
	      for (int k = 0; k < normalvecs.Size(); k++)
		{
		  ...
		  if (
		      dynamic_cast<const Polyhedra*> (ageometry.surf2prim[surfind[j]]) && 
...
#ifdef DEVELOP
		  std::cout << "           tangential vector " << t << endl;
#endif
...
#ifdef DEVELOP
		      std::cout << "cnt_tang = " << cnt_tang_faces << endl;
#endif
	...
	if(same.Size() < 3)
	  for(int j=0; j<same.Size(); j++)
	    {
	      std::cout << "setting " << specpoints[same[j]].p << "; " << specpoints[same[j]].v << "; " 
