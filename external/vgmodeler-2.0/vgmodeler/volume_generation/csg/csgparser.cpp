...
namespace netgen
{
  ...
  enum TOKEN_TYPE
    { 
      ...
      TOK_SOLID, TOK_RECO, TOK_TLO, TOK_CURVE2D, TOK_BOUNDINGBOX, TOK_BOUNDARYCONDITION,
      TOK_END };
	...
	switch (scan.GetPrimitiveToken())
	  ...
	  case TOK_ORTHOBRICK:
	      ...
	      for (int j = 0; j < 6; j++)
		{
		  ...
		  switch (j)
		    {
		    case 0: plane = new Plane (pa, Vec<3>(-1, 0, 0)); break;
		    case 1: plane = new Plane (pa, Vec<3>(0, -1, 0)); break;
		    case 2: plane = new Plane (pa, Vec<3>(0, 0, -1)); break;
		    case 3: plane = new Plane (pb, Vec<3>(1, 0, 0)); break;
		    case 4: plane = new Plane (pb, Vec<3>(0, 1, 0)); break;
		    case 5: plane = new Plane (pb, Vec<3>(0, 0, 1)); break;
		    }
		  geom -> AddSurfaces (plane);
		  
		  if (!sol)
		    sol = new Solid (plane);
		  else
		    sol = new Solid (Solid::SECTION, sol, new Solid (plane));
		}
	      return sol;
	      */
	    } 
  ...
  */
  CSGeometry * ParseCSG (istream & istr)
  {
		  ...
		  case TOK_EDGE:
		    {
		      ...
		      cout << "Singular Edge  with factor " << factor << endl; 
    ...
    std::cout << geom->GetNTopLevelObjects() << " TLOs:" << endl;
    for (int i = 0; i < geom->GetNTopLevelObjects(); i++)
      {
	const TopLevelObject * tlo = geom->GetTopLevelObject(i);
	if (tlo->GetSolid())
	  std::cout << i << ": " << *tlo->GetSolid() << endl;
      }

    std::cout << geom->GetNSurf() << " Surfaces" << endl;
    for (int i = 0; i < geom->GetNSurf(); i++)
      std::cout << i << ": " << *geom->GetSurface(i) << endl;
