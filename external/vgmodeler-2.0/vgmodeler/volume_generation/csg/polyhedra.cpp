...
INSOLID_TYPE Polyhedra :: PointInSolid (const Point<3> & p,
					double eps) const
{
  //std::cout << "PointInSolid p " << p << " eps " << eps << endl;
  //std::cout << "bbox " << poly_bbox << endl;
...
//       if(writeit)
// 	{
// 	  std::cout << "test 1 " << (fabs (lam3) > eps) << endl;
// 	  std::cout << "v0 " << v0 << " faces[i].nn " << faces[i].nn << " lam3 " << lam3 << " eps " << eps << endl;
// 	}
...
INSOLID_TYPE Polyhedra :: VecInSolid2 (const Point<3> & p,
				       ...
				       double eps) const
{
  //std::cout << "VecInSolid2 eps " << eps << endl;
  INSOLID_TYPE res = VecInSolid(p,v1,eps);
  //std::cout << "VecInSolid = " <<res <<endl;
  ...
  std::cout << "primitive::vecinsolid2 makes nonsense for polyhedra" << endl;
  cerr << "primitive::vecinsolid2 makes nonsense for polyhedra" << endl;
				    ...
				    ARRAY<double> & coeffs) const
{
  ...
  for (i = 1; i <= planes.Size(); i++)
    {
      planes.Elem(i)->Print std::cout;
    }
  for (i = 1; i <= faces.Size(); i++)
    {
      std::cout << "face " << i << " has plane " << faces.Get(i).planenr << endl;
      for (j = 1; j <= 3; j++)
	std::cout << points.Get(faces.Get(i).pnums[j-1]);
      std::cout << endl;
    }
  */
}
...
{
  std::cout << "polyhedra, add face " << pi1 << ", " << pi2 << ", " << pi3 << endl;
  ...
  std::cout << "is plane nr " << faces.Last().planenr << endl;
...
{
  /*
  std::cout << "check face box intersection, fnr = " << fnr << endl;
  std::cout << "box = " << box << endl;
  std::cout << "face-box = " << faces[fnr].bbox << endl;
  */
  ...
  std::cout << "box.Center() = " << box.Center() << endl;
  std::cout << "center = " << box.Center() << endl;
  std::cout << "dist2 = " << dist2 << endl;
  std::cout << "diam = " << box.Diam() << endl;
  */
  if (dist2 < sqr (box.Diam()/2))
    {
...
Vec<3> Polyhedra :: SpecialPointTangentialVector (const Point<3> & p, int s1, int s2) const
{
  ...
  for (int fi1 = 0; fi1 < faces.Size(); fi1++)
	...
	//std::cout << "check pair fi1/fi2 " << fi1 << "/" << fi2 << endl;
	...
	for(int j = 0; !shareedge && j < 3; j++)
	  {
	    ...
	    for (int k = 0; !shareedge && k < 3; k ++)
		 ...
		 //std::cout << "v2.Length2() " << v2.Length2() << endl;
		 ...
		 //std::cout << "sa " << sa << " sb " << sb << " smax " << smax << " sp " << sp  << " v1 " << v1 << endl;
		 //testout->precision(8);
