...
namespace vgmodeler
{
  ...
  { 
    std::cout << "Adding surface " << name << endl;
  ...
  */
	...
	if (GetSurface(j) -> IsIdentic (*GetSurface(i), inv, eps))
	  {
	    ...
	    // std::cout << "surfaces " << i2 << " are identic" << endl;
	  }

    std::cout << "identicmap:" << endl;
    for (int i = 0; i < isidenticto.Size(); i++)
      std::cout << i << " -> " << isidenticto[i] << endl;

    /*    
    for (int i = 0; i < nsurf; i++)
      GetSurface(i)->Print std::cout;
    */
  }
			    ...
			    double detail, double facets)
  {
	...
	for (int j = 0; j < GetNSurf(); j++)
	  // for (int jj = 0; jj < surfind.Size(); jj++)
	  {
		...
		tams -> AddNormal (n);
		//std::cout << "point " << tas.GetPoint(k) << " normal " << n << endl;
			...
			IndexSet & iset)
  {
    ...
    //std::cout << "tria " << tams.GetPoint(tria[0]) << " - " << tams.GetPoint(tria[1]) << " - " << tams.GetPoint(tria[2]) 
    ...
    if (cntindep == 1)
      {
	tams.AddTriangle (tria);
	//std::cout << "pos1 " << tams.GetPoint(tria[0]) << " - " << tams.GetPoint(tria[1]) << " - " << tams.GetPoint(tria[2]) << endl;
	return;
      }
...
// 		      }
		...
		//std::cout << "pos2 " << tams.GetPoint(ntria[0]) << " - " << tams.GetPoint(ntria[1]) << " - " << tams.GetPoint(ntria[2]) << endl
