...
namespace vgmodeler
{
  ...
  ADTreeNode3 :: ADTreeNode3()
  {
    ...
    // [FS] .. try to avoid static
//     std::cout << ".. [FS] test: " << sizeof (ADTreeNode3) << std::endl;
//     ball = new BlockAllocator();
  }
...
//     ball.Free (p);
  ...
  void ADTree3 :: GetIntersecting (const float * bmin, 
				   ...
				   ARRAY<int> & pis) const
  {
     ARRAY<ADTreeNode3*> stack(1000);
     ARRAY<int> stackdir(1000);
    ADTreeNode3 * node;
    int dir, stacks;
  ...
  void ADTree3Div :: Insert (const float * p, int pi)
  {
	...
	/*      
		std::cout << "bmin, max = " << bmin[dir] << "-" << bmax[dir]
		<< " p = " << p[dir];
	*/
	...
	//      std::cout << "new bmin, max = " << bmin[dir] << "-" << bmax[dir] << endl;
  ...
  void ADTree3Div :: GetIntersecting (const float * bmin, 
				      ...
				      ARRAY<int> & pis) const
  {
     ARRAY<ADTreeNode3Div*> stack(1000);
     ARRAY<int> stackdir(1000);
    ADTreeNode3Div * node;
    int dir, i, stacks;
	...
	//      std::cout << "get int, mini, maxi = " << mini << ", " << maxi << endl;
	if (mini < 0) mini = 0;
  ...
  void ADTree3FM :: GetIntersecting (const float * bmin, 
				     ...
				     ARRAY<int> & pis) const
  {
     ARRAY<ADTreeNode3FM*> stack(1000);
    ADTreeNode3FM * node;
    int dir, i, stacks;
  ...
  /*
    ...
    ARRAY<int> & pis) const
    {
     ARRAY<ADTreeNode6*> stack(10000);
     ARRAY<int> stackdir(10000);
    ADTreeNode6 * node;
    int dir, stacks;
