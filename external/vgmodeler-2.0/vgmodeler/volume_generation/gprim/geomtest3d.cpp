  ...
  */
  ...
  if (fabs (det) <= 1e-10 * arel)
    {
      ...
      //      std::cout << "WARNING: IntersectTriangleLine degenerated\n";
      /*
      std::cout << "lin-tri intersection: " << endl
		 << "line = " << *line[0] << " - " << *line[1] << endl
		 << "tri = " << *tri[0] << " - " << *tri[1] << " - " << *tri[2] << endl
		 << "lami = " << lami << endl
  ...
  if (
...

     {
      ...
      std::cout << "WARNING: IntersectTriangleLine numerical inexact" << endl;
			  ...
			  const int * tetpi, const int * tripi)
{
  ...
  */
  ...
  //  std::cout << "tet-trig inters, cnt = " << cnt << endl;
...
//GH	break;
  ...
  MARK (inttettric2);
	...
	//	std::cout << "trip1,2,3 = " << trip1 << ", " << trip2 << ", " << trip3 << endl;
	//	std::cout << "tetp1,2,3,4 = " << tetp1 << ", " << tetp2 
	//		   << ", " << tetp3 << ", " << tetp4 << endl;
	...
	if (lam1 < -epsrel || lam2 < -epsrel)
	...
	else
	  {
	    ...
	    std::cout << "vtet1 = " << vtet1 << endl;
	    std::cout << "vtet2 = " << vtet2 << endl;
	    std::cout << "vtri = " << vtri << endl;
	    std::cout << "lam1 = " << lam1 << " lam2 = " << lam2 << endl;
	    std::cout << (lam1 * (vtet1 * vtet1) + lam2 * (vtet1 * vtet2))
		       << " = " << (vtet1 * vtri) << endl;
	    std::cout << (lam1 * (vtet1 * vtet2) + lam2 * (vtet2 * vtet2))
		       << " = " << (vtet2 * vtri) << endl;
...
int IntersectTetTriangleRef (const Point3d ** tri, const int * tripi)
{
  ...
  double eps2 = eps * eps;
   ...
   Point3d rtetp1(0, 0, 0);
   Point3d rtetp2(1, 0, 0);  
   Point3d rtetp3(0, 1, 0); 
   Point3d rtetp4(0, 0, 1);

   const Point3d * tet[] = { &rtetp1, &rtetp2, &rtetp3, &rtetp4 };
   int tetpi[] = { 1, 2, 3, 4 };
	...
	for (i = 0; i <= 2; i++)
	  {
	    ...
	    if (IntersectTriangleLine (tri1, &line[0]))
	      {
		std::cout << "int1, line = " << *line[0] << " - " << *line[1] << endl;
		return 1;
	      }
	  }	
	...
	for (i = 0; i <= 2; i++)
	  {
	    ...
	    if (IntersectTriangleLine (tri2, &line[0]))
	      {
		std::cout << "int2, line = " << *line[0] << " - " << *line[1] << endl;
		return 1;
	      }
	  }	
	break;
...
int CalcSphereCenter (const Point3d ** pts, Point3d & c)
{
  ...
  if (fabs (a.Det()) <= 1e-12 * h * h * h)
    {
      std::cout << "CalcSphereCenter: degenerated" << endl;
      return 1;
    }
...
int CalcTriangleCenter (const Point3d ** pts, Point3d & c)
{
  ...
  if (fabs (a.Det()) <= 1e-12 * h * h)
    {
      std::cout << "CalcTriangleCenter: degenerated" << endl;
      return 1;
    }
