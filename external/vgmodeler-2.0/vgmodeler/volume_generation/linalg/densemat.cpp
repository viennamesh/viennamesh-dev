...
DenseMatrix & DenseMatrix :: operator= (const BaseMatrix & m2)
  ...
  if (data)
    ...
    std::cout << "DenseMatrix::Operator=: Matrix not allocated" << endl;
  ...
  */
  ...
  DenseMatrix & DenseMatrix :: operator+= (const DenseMatrix & m2)
  {
    ...
    {
      std::cout << "DenseMatrix::Operator+=: Sizes don't fit" << endl;
      return *this;
    }
...
double & DenseMatrix :: operator() (int i, int j)
{
  if (i >= 1 && j >= 1 && i <= height && j <= width)
    return Elem(i,j);
  else std::cout << "DenseMatrix: index (" << i << "," << j << ") out of range (1.."
		<< height << ",1.." << width << ")\n";
   double dummy = 0;
  return dummy;
}
...
double DenseMatrix :: Det () const
  ...
  if (width != height)
    {
    std::cout << "DenseMatrix :: Det: width != height" << endl;
    return 0;
    }
  ...
  switch (width)
    ...
    default:
      {
      std::cout << "Matrix :: Det:  general size not implemented (size=" << width << ")" << endl;
      return 0;
      }
    }
  }
...
void CalcInverse (const DenseMatrix & m1, DenseMatrix & m2)
  ...
  if (m1.width != m1.height)
    {
    std::cout << "CalcInverse: matrix not symmetric" << endl;
    return;
    }
  if (m1.width != m2.width || m1.height != m2.height)
    {
    std::cout << "CalcInverse: dim(m2) != dim(m1)" << endl;
    return;
    }
  ...
  if (m1.Width() <= 3)
    ...
    if (det == 0)
      {
      std::cout << "CalcInverse: Matrix singular" << endl;
      return;
      }
...
void CalcAAt (const DenseMatrix & a, DenseMatrix & m2)
  ...
  if (m2.Height() != n1 || m2.Width() != n1)
    {
    std::cout << "CalcAAt: sizes don't fit" << endl;
    return;
    }
...
BaseMatrix * DenseMatrix :: InverseMatrix (const BitArray * /* inner */) const
{
  if (Height() != Width())
    {
      std::cout << "BaseMatrix::InverseMatrix(): Matrix not symmetric" << endl;
	...
	{	
#ifdef DEBUG
	  std::cout << "Invmat not available" << endl;
#endif
...
void CalcAtA (const DenseMatrix & a, DenseMatrix & m2)
  ...
  if (m2.Height() != n2 || m2.Width() != n2)
    {
    std::cout << "CalcAtA: sizes don't fit" << endl;
    return;
    }
...
void CalcABt (const DenseMatrix & a, const DenseMatrix & b, DenseMatrix & m2)
  ...
  if (m2.Height() != n1 || m2.Width() != n3 || b.Width() != n2)
    {
    std::cout << "CalcABt: sizes don't fit" << endl;
    return;
    }
...
void CalcAtB (const DenseMatrix & a, const DenseMatrix & b, DenseMatrix & m2)
  ...
  if (m2.Height() != n2 || m2.Width() != n3 || b.Height() != n1)
    {
    std::cout << "CalcAtB: sizes don't fit" << endl;
    return;
    }
...
DenseMatrix operator* (const DenseMatrix & m1, const DenseMatrix & m2)
  ...
  if (m1.Width() != m2.Height())
    {
    std::cout << "DenseMatrix :: operator*: Matrix Size does not fit" << endl;
    }
  else if (temp.Height() != m1.Height())
    {
    std::cout << "DenseMatrix :: operator*: temp not allocated" << endl;
    }
  else
    {
    Mult (m1, m2, temp);
...
void Mult (const DenseMatrix & m1, const DenseMatrix & m2, DenseMatrix & m3)
  ...
  if (m1.Width() != m2.Height() || m1.Height() != m3.Height() ||
       m2.Width() != m3.Width() )
    {
    std::cout << "DenseMatrix :: Mult: Matrix Size does not fit" << endl;
    std::cout << "m1: " << m1.Height() << " x " << m1.Width() << endl;
    std::cout << "m2: " << m2.Height() << " x " << m2.Width() << endl;
    std::cout << "m3: " << m3.Height() << " x " << m3.Width() << endl;
    return;
    }
  /*
  else if (m1.Symmetric() || m2.Symmetric() || m3.Symmetric())
    {
    std::cout << "DenseMatrix :: Mult: not implemented for symmetric matrices" << endl;
    return;
    }
  */
  else
...
double DenseMatrix :: EvaluateBilinearform (const Vector & hx) const
  ...
  if (Width() != hx.Size() || Height() != hx.Size())
    {
    std::cout << "Matrix::EvaluateBilinearForm: sizes don't fit" << endl;
    }
  else
    {
    for (i = 1; i <= Height(); i++)
