...
double & SparseMatrix :: operator() (INDEX i, INDEX j)
{
  if (i >= 1 && i <= height && j >= 1 && j <= width)
    ...
    {
    std::cout << "SparseMatirx::operator(): out of range" << endl;
    return shit;
    }
}
...
void SparseMatrix :: Mult (const Vector & v, Vector & prod) const
  ...
  if (prod.Size() != Height() || v.Size() != Width())
    {
    std::cout << "SparseMatrix::Mult: Dimensions don't fit" << endl;
    return;
    }
    */
...
void SparseMatrix :: Residuum (const Vector & bx, const Vector & bb,
  ...
  if (res.Size() != b.Size() || b.Size() != Height() ||
      x.Size() != Width())
    {
    std::cout << "SparseMatrix::Residuum: Dimensions don't fit" << endl;
    return;
    }
...
void SparseMatrix :: ResiduumTrans (const Vector & /* bx */, 
  ...
  if (res.Size() != b.Size() || b.Size() != Width() ||
      x.Size() != Height())
    {
    std::cout << "SparseMatrix::ResiduumTrans: Dimensions don't fit" << endl;
    return;
    }
...
{
  std::cout << "SparseMatrixFix cannot change graph, requested: " 
       << i << "-" << j << endl;
  return 0;
}
...
void SparseMatrix :: SetSize (INDEX h, INDEX w)
  ...
  if (lins.Size () == height)
    ...
    {
    height = width = 0;
    std::cout << "SparseMatrix::SetSize: Out of memory" << endl;
    }
  }
  ...
  if (size)
    ...
    if (j > lin->col[size-1].colnr)
      ...
      if ( (ncol = NewColStruct(lin->maxsize+4)/* new colstruct[lin->maxsize+4] */) != NULL)
        ...
        {
        std::cout << "SparseMatrix::Elem: Out of memory 1" << endl;
        return shit;
        }
...
//        delete lin->col;
        ...
        {
        std::cout << "SparseMatrix::Elem: Out of memory 2" << endl;
        return shit;
        }
      }
    }
    ...
    if (lin->maxsize)
      ...
      if ( (lin->col = NewColStruct(6) ) != NULL )
        ...
        {
        std::cout << "SparseMatrix::Elem: Out of memory 3" << endl;
        return shit;
        }
      }
    }
...
void SparseMatrix :: SetLineAllocSize (INDEX i, int j)
  ...
  if (lins[i].maxsize < j)
    ...
    if ( (ncol = NewColStruct(j)) != NULL)
      ...
      std::cout << "SPARSE_MATIRX :: SetLineAllocSize: Out of Memory" << endl;
    }
  }
...
SparseMatrix operator* (const SparseMatrix & m1,
  ...
  if (!m1.Symmetric() && !m2.Symmetric())
    ...
    {
    std::cout << "SparseMatrix :: operator* not implemented for symmetric case" << endl;
    }
  return m;
  }
...
SparseMatrix & SparseMatrix :: operator+= (const SparseMatrix & m2)
  ...
  if (Symmetric() == m2.Symmetric())
    ...
    {
    std::cout << "SparseMatrix :: operator+= not implemented for different cases" << endl;
    }
  return *this;
  }
...
SparseMatrix & SparseMatrix :: operator*= (const SparseMatrix & m2)
  ...
  if (!Symmetric() && !m2.Symmetric())
    ...
    {
    std::cout << "SparseMatrix :: operator*= not implemented for Symmetric matrices" << endl;
    }
  ...
  if (!Symmetric())
    ...
    for (i = 1; i <= Height(); i++)
      ...
      if (temp.ElementsInLine(i) < 1 || temp.GetIndex(i, 1) != i)
        {
        std::cout << "i = " << i << endl;
        std::cout << "Solve: Matrix singular" << endl;
        char ch;
        cin >> ch;
        }
      for (j = 2; j <= temp.ElementsInLine(i); j++)
        ...
        if (temp.GetIndex(nr, 1) != i)
          {
          std::cout << temp << endl;
          std::cout << "i = " << i << "j = " << j << "nr = " << nr << endl;
          std::cout << "Solve: Graph not symmetrix" << endl;
          char ch;
          cin >> ch;
          }
    ...
    std::cout << "SparseMatrix :: Solve not implemented for symmetic case" << endl;
  }
...
//  return;
  ...
  if (i > poolsizes.Size())
    {
    std::cout << "SparseMatrix :: DeleteColStruct: Size not found" << endl;
    return;
    }
