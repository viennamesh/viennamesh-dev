...
//       cout << "new: no inner point found" << endl;
    ...
    std::cout << "hi(orig) = " << hi << ", hi(new) = " << hi2;
    if (hi != hi2) std::cout << "hi different" << endl;
    */
...
FuncDeriv (const Vector & x, const Vector & dir, double & deriv) const
{
  ...
  /*
    std::cout << "bad1 = " << badness << " der = " << deriv << endl;
    ...
    std::cout << "bad2 = " << Func(x) << " der = " << deriv << endl;
...
void Mesh :: ImproveMesh (const CSGeometry & geometry, OPTIMIZEGOAL goal)
{
  ...
  //  std::cout << "Vol = " << CalcVolume (points, volelements) << endl;
  ...
  for (i = 1; i <= points.Size(); i++)
	...
	if (i % 10 == 0)
      ...
      //    std::cout << "Now point " << i << "\n";
      //    std::cout << "Old: " << points.Get(i) << "\n";
      ...
      if (!surf1)
	{
      ...
      //    std::cout << "New Point: " << points.Elem(i) << "\n" << "\n";
...
void Mesh :: ImproveMeshJacobian (OPTIMIZEGOAL goal)
{
  ...
  std::cout << "Improve Mesh Jacobian" << "\n";
  ...
  for (i = 1; i <= points.Size(); i++)
	...
	std::cout << "improvejac, p = " << i << endl;

	if (goal == OPT_WORSTCASE && !badnodes.Test(i))
	  continue;
	//	std::cout << "smoot p " << i << endl;
