...
namespace vgmodeler
{
  ...
  /*
   const double c_trig = sqrt(3.0) / 12;
   const double c_trig4 = sqrt(3.0) / 3;
  */
   const double c_trig = 0.14433756;
   const double c_trig4 = 0.57735026;
			      ...
			      double h)
  {
   ...
   ARRAY<int> locrots(0);
   ARRAY<double> lochs(0);
  //  int locerr2;
   double locmetricweight = 0;
   double loch;
   int surfi, surfi2;
   int uselocalh;
  ...
  FuncGrad (const Vector & x, Vector & grad) const
  {
	...
	if (Determinant(e1, e2, n) > 1e-8 * e1l * e2.Length())
	  ...
	  {
	    std::cout << "very very bad badness" << endl;
	    badness += 1e8;
	  }
      }
  ...
  FuncDeriv (const Vector & x, const Vector & dir, double & deriv) const
  {
    ...
    vgrad.Add (-(vgrad * n), n);
    deriv = dir.Get(1) * (vgrad*t1) + dir.Get(2) * (vgrad*t2);
...
//      int timer = NgProfiler::CreateTimer ("MeshSmoothing 2D");
//     NgProfiler::RegionTimer reg (timer);
    ...
    */
      ...
      if (mesh[pi].Type() == SURFACEPOINT)
	{
	  ...
	  if (mixed)
	    {
	      //	      std::cout << "vorher : " << surfminfj.Func (x) << endl;
	      BFGS (x, surfminfj, par, 1e-6);
	      //	      std::cout << "nachher: " << surfminfj.Func (x) << endl;
	      //	      std::cout << "x = " << x << endl;
	    }
	  else
	    {
	      //	      std::cout << "vorher : " << surfminf.Func (x) << endl;
	      BFGS (x, surfminf, par, 1e-6);
	      //	      std::cout << "nachher: " << surfminf.Func (x) << endl;
	      //	      std::cout << "x = " << x << endl;
	    }
