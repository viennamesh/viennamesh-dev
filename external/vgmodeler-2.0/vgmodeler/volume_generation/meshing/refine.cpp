...
namespace vgmodeler
{
  ...
  {
    std::cout << "here .. in refine. " << std::endl;
    // reduce 2nd order
	...
	while (lam < 0.9 && cnttrials > 0)
	  {
		...
		std::cout << "smooth points: " << endl;
		for (int i = 1; i <= free.Size(); i++)
		  if (free.Test(i))
		    std::cout << "p " << i << endl;

		std::cout << "surf points: " << endl;
		for (int i = 1; i <= mesh.GetNSE(); i++)
		  for (int j = 1; j <= 3; j++)
		    std::cout << mesh.SurfaceElement(i).PNum(j) << endl;
		...
		for (int i = 1; i <= mesh.GetNE(); i++)
		  {
			...
			std::cout << "wrong el: ";
			for (int j = 1; j <= 4; j++)
			  std::cout << mesh.VolumeElement(i).PNum(j) << " ";
			std::cout << endl;
		      }
		    else
		      mesh.VolumeElement(i).flags.badel = 0;
