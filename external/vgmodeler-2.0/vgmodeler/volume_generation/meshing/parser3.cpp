...
void vnetrule :: LoadRule (istream & ist)
{
  ...
  /*
  std::cout << "Rule " << Name() << endl;
  std::cout << "oldutofreezone = " << (*oldutofreezone) << endl;
  std::cout << "oldutofreezonelimit = " << (*oldutofreezonelimit) << endl;
  */
  ...
  //  for (i = 1; i <= freezone.Size(); i++)
  //    std::cout << "freepoint: " << freezone.Get(i) << endl;
  ...
  for (fs = 1; fs <= freesets.Size(); fs++)
	...
	for (ii2 = 1; ii2 <= freeset.Size(); ii2++)
	  for (ii3 = 1; ii3 <= freeset.Size(); ii3++)
		...
		n = Cross (v1, v2);
		n /= n.Length();
		//		std::cout << "i1,2,3 = " << i1 << ", " << i2 << ", " << i3 << endl;
		//		std::cout << "v1 = " << v1 << " v2 = " << v2 << " n = " << n << endl;
		ok = 1;
...

  {
    ...
    // std::cout << "rule " << name << ", pnear = " << pnearness << endl;
  }
      ...
      for (k = 1; k <= freesetfaces.Size(); k++)
	{
	  ...
	  for (l = k+1; l <= freesetfaces.Size(); l++)
		...
		for (int f12 = 1; f12 <= 3; f12++)
	      ...
	      //	      std::cout << "edge = " << edge.I(1) << "-" << edge.I(2) << endl;
	      //	      std::cout << "ind = " << ind << " edge = " << edge << endl;
		  ...
		  if (GetNP(eli) == 4)
		      ...
		      for (int elr = 1; elr <= 4; elr++)
			{
			  if (GetPointNrMod (eli, elr) == edge.I(1) &&
			    ...
			    {
			      /*
			      std::cout << "edge is diagonal of rectangle" << endl;
			      std::cout << "edge = " << edge.I(1) << "-" << edge.I(2) << endl;
  ...
  if (filename)
  ...
  else 
    {
      /* connect tetrules to one string */
       //std::cout << " \t .. use internal rules " << std::endl;
