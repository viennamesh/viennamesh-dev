				  ...
				  const Element2d & elem)
{
  ...
  if (testmode)
    {
      std::cout << "l = " << l12 << " + " << l13 << " + " << l23 << " = " 
		 << cir << ", area = " << area << endl;
      std::cout << "shapeerr = " << 10 * (c * cir * cir / area - 1) << endl
			   ...
			   ARRAY<INDEX> & dellines, int tolerance)
{
  ...
  int loctestmode;
   ...
   ARRAY<int> pused, lused;
   ARRAY<int> pmap, lmap, pfixed;
   ARRAY<int> pnearness, lnearness;
  
   ARRAY<Point2d> tempnewpoints;
   ARRAY<INDEX_2> tempnewlines;
   ARRAY<int> tempdellines;
   ARRAY<Element2d> tempelements;
	  ...
	  if (nlok <= rule->GetNOldL())
	      ...
	      while (npok >= 1)
		{
		  ...
		  if (npok <= rule->GetNOldP())
			...
			{
			  ok = 0;
			  if (loctestmode) std::cout << "freezone not convex" << endl;
			  ...
			  if (elerr < minelerr)
			      ...
			      if (loctestmode)
				{
				  ...
				  for (i = 1; i <= rule -> GetTransFreeZone().Size(); i++)
				    std::cout << rule->GetTransFreeZone().Get(i) << endl;
				}
