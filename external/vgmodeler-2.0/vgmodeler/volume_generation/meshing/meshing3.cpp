...
{
//    int meshing3_timer = NgProfiler::CreateTimer ("Meshing3::GenerateMesh");
//    int meshing3_timer_a = NgProfiler::CreateTimer ("Meshing3::GenerateMesh a");
//    int meshing3_timer_b = NgProfiler::CreateTimer ("Meshing3::GenerateMesh b");
//    int meshing3_timer_c = NgProfiler::CreateTimer ("Meshing3::GenerateMesh c");
//    int meshing3_timer_d = NgProfiler::CreateTimer ("Meshing3::GenerateMesh d");
//   NgProfiler::RegionTimer reg (meshing3_timer);
  ...
  ARRAY<INDEX> delpoints, delfaces;   // points and lines to be deleted
...
//      hshould = mesh.GetH (pmid);
      double factor=1;
      if (mp.increase_level >= 1)
         factor = 0;
      hshould = mesh.GetH (pmid) + mesh.GetH (pmid) / 100 * factor;   // [RH] .. change the surrounding area calc..
...
//       NgProfiler::StopTimer (meshing3_timer_a);
      ...
      // std::cout << "locfaces = " << endl << locfaces << endl;
...
// 	{
#ifdef DEBUG
	  std::cout << "baseel = " << baseelem << ", ind = " << findex.Get(1) << endl;
	  std::cout << "pi = " << pi1 << ", " << pi2 << ", " << pi3 << endl;
	  
	  std::cout << "baseel: p1: " << bel.PNum(1) << ", p2: " << bel.PNum(2) << ", p3: " << bel.PNum(3) <<std::endl;
	  std::cout << std::setprecision(8);
	  std::cout << "p1: " << p1 << ", p2: " << p2 << ", p3: " << p3 << endl;
...
// 	{
#ifdef DEBUG
	  std::cout << "baseelem = " << baseelem << " qualclass = " << stat.qualclass << endl;
	  std::cout << "locpoints = " << endl << locpoints << endl;
	  std::cout << "connected = " << endl << connectedpairs << endl;
#endif
// 	}
	...
	allowpoint = 2;
	  ...
	  mp.baseelnp != 4)   
	{
// 	  NgProfiler::RegionTimer reg (meshing3_timer_b);
	  ...
	  if (onlytri && groupfaces.Size() <= 20 + 2*stat.qualclass &&
	    ...
	    {
#ifdef DEBUG
	      std::cout << "inner point found" << endl;
#endif
      ...
      for (i = 1; i <= locfaces.Size(); i++)
	{
	  ...
	  for (j = 1; j <= locfaces.Get(i).GetNP(); j++)
	    std::cout << pindex.Get(locfaces.Get(i).PNum(j)) << " ";
	  std::cout << endl;
	}
...
// 	  NgProfiler::RegionTimer reg (meshing3_timer_d);	  
	  
// 	  if (loktestmode)
// 	    {
#ifdef DEBUG
	      std::cout << "plainpoints = " << endl << plainpoints << endl;
	      std::cout << "baseel: p1: " << bel.PNum(1) << ", p2: " << bel.PNum(2) << ", p3: " << bel.PNum(3) <<std::endl;
	      std::cout << "Applyrules found " << found << endl;
#endif
// 	    }
	  ...
	  if (stat.cntelem != plotstat_oldne)
	    {
	      plotstat_oldne = stat.cntelem;
 	      ...
 	      PrintMessageCR (2, "\t processed elements: ", stat.cntelem,
 			      //	    << " trials: " << stat.cnttrials
 			      //" faces: ", stat.nff,
 			      " vol = ", float(100 * stat.vol / stat.vol0));
	      // [RH] output
	      //
//               if (stat.cntelem % 500 == 0)
//                  std::cout << "  .. meshing process: " << stat.cntelem << " / " << 100 * stat.vol / stat.vol0 << std::endl;;
	  ...
	  if (found && (!hasfound || err < minerr) )
	    {
	      ...
	      if (testmode)
		{
		  std::cout << "found is active, 3" << endl;
		  for (i = 1; i <= plainpoints.Size(); i++)
		    {
		      std::cout << "p";
		      if (i <= pindex.Size())
			std::cout << pindex.Get(i) << ": ";
		      else
			std::cout << "new: ";
		      std::cout << plainpoints.Get(i) << endl;
		    }
		}
...
#ifdef DEBUG
      std::cout << " .. after for: rotindex .. " << std::endl;
#endif      
      ...
      if (hasfound)
	{
	  ...
	  /*
	  if (optother)
	    std::cout << "Other is optimal" << endl;

	  if (minother < minwithoutother)
	    {
	      std::cout << "Other is better, " << minother << " less " << minwithoutother << endl;
	    }
	    */
	  ...
	  for (i = oldnf+1; i <= locfaces.Size(); i++)
		...
		locfaces.Elem(i).PNum(j) = 
	      ...
	      // std::cout << "add face " << locfaces.Get(i) << endl;
      ...
      else
	{
	  ...
	  if (impossible && mp.check_impossible)
	    {
	      std::cout << "skip face since it is impossible" << endl;
	      for (int j = 0; j < 100; j++)
		adfront->IncrementClass (findex.Get(1));
	    }
	}
  ...
  for (i = 1; i <= ruleused.Size(); i++)
    std::cout << setw(4) << ruleused.Get(i)
...
void Meshing3 :: BlockFill (Mesh & mesh, double gh)
{
 ...
 const AdFront3 * locadfront;
 int TestInner (const Point3d & p)
{
  return locadfront->Inside (p);
}
 int TestSameSide (const Point3d & p1, const Point3d & p2)
{
...
maxh);
  ...
  if (debugparam.slowchecks)
    std::cout << "Blockfill with points: " << endl;
  for (i = 1; i <= npoints.Size(); i++)
    {
