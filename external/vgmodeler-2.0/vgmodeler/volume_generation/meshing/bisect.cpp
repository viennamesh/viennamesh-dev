      ...
      do
	{
	  ...
	  for (i = 1; i <= mesh.GetNE(); i++)
	      ...
	      for (j = 0; j < 3; j++)
		{
		  ...
		  //		  std::cout << "identify edges " << eclass1 << "-" << eclass2 << endl;
...
// =====================================
// 
// RH bisected refinement
//
void Refinement :: Bisect (Mesh & mesh, BisectionOptions & opt)
{
  ...
  */
  ...
  for (l = 1; l <= 1; l++)
      ...
      if (opt.refinementfilename)
	{
	  ...
	  std::cout << "# RH .. load refine information.. use marked elements "  << endl;
	...
	{
	  std::cout << "# RH .. refine.. use marked elements "  << endl;
	...
	{
	  std::cout << "### RH refine .. btmarktets : " << std::endl;
	  ...
	  if (opt.refine_p)
	    {
	      std::cout << "# RH refine .. opt.refine_p : " << std::endl;
	      cout << "refine p" << endl;
      ...
      // =========
      // 
      // control flow of the subsequent code party ;-)
      //
      // RH .. part 1) create new tetrahedrons
      // RH .. part 2) create new triangles
      // RH .. part 3) create new quads
      // RH .. part 4) hanging in edge
      //
      do
	{
	  // refine volume elements
	  std::cout << "## RH: refine volume elements.. " << std::endl;
	  ...
	  for (i = 1; i <= nel; i++)
		...
		if (cutedges.Used (edge))
		  ...
		  {
		    // RH .. a new point is created and inserted..
		    //
	  ...
	  for (i = 1; i <= nquad; i++)
		...
		std::cout << "project point " << newp1 << " old: " << mesh.Point(newp1);
		PointBetween (mesh.Point (edge1.I1()), mesh.Point (edge1.I2()),
			      0.5, si,
			      oldquad.pgeominfo[0],
			      oldquad.pgeominfo[1],
			      mesh.Point (newp1), npgi1);
		std::cout << " new: " << mesh.Point(newp1) << endl;
	  ...
	  for (i = 1; i <= nseg; i++)
	      ...
	      if (cutedges.Used (edge))
		{
		  ...
		  //		  std::cout << "move edgepoint " << newpi << " from " << mesh.Point(newpi);
		  ...
		  //		  std::cout << " to " << mesh.Point (newpi) << endl;
  ...
  cout << "## RH mem in mtets: " << endl;
  mtets.PrintMemInfo(cout);
  cout << "## RH cutedges" << endl;
  cutedges.PrintMemInfo(cout);

  //std::cout << "## RH mtets = " << mtets << endl;
  ...
  std::cout << "## RH mprism size: " << mprisms.Size() << std::endl;
  std::cout << "## RH tets size: "   << mtets.Size() << std::endl;
  for (i = 1; i <= mtets.Size(); i++)
    {
      Element el(TET);
  ...
  for (i = 1; i <= mprisms.Size(); i++)
	...
	case 2:
	  {
	     const int tetmap1[] = { 1, 2, 3, 4 };
	     const int tetmap2[] = { 2, 3, 1, 5 };
	     const int tetmap3[] = { 3, 1, 2, 6 };
	    if (!deg1) map = tetmap1;
	    if (!deg2) map = tetmap2;
	    if (!deg3) map = tetmap3; 
	    for (j = 1; j <= 4; j++)
  ...
  */
  std::cout << "## RH  cutedge side: " << cutedges.Size() << std::endl;
...
//       std::cout << "connection table: " << endl;
//       for (i = 1; i <= np; i++)
//       {
//       std::cout << "node " << i << ": ";
// 	  for (j = 1; j <= conto.EntrySize(i); j++)
// 	  std::cout << conto.Get(i, j) << " ";
// 	  std::cout << endl;
// 	}
...
*/  
  ...
  mesh.ComputeNVertices(); // number of vertices with maximum search
  mesh.UpdateTopology();   // topology->update()  ..clusters->update()  ..
