  ...
  if (goal == OPT_QUALITY)
    {
      ...
      std::cout << "Total badness = " << totalbad << endl;
      ...
      for (int j = 0; j < 6; j++)
	{
	  ...
	  if (bad2 / hasonepi.Size()  < 
	      ...
	      for (int k = 0; k < row.Size(); k++)
		{
		  ...
		  elem.flags.illegal_valid = 0;
		  if (!mesh.LegalTet (elem))
		    std::cout << "illegal tet " << elementsonnode[pi2][k] << endl;
		}
				     ...
				     OPTIMIZEGOAL goal)
{
  ...
  if (goal == OPT_QUALITY)
    {
      ...
      std::cout << "Total badness = " << bad1 << endl;
    }
  ...
  if (goal == OPT_QUALITY || goal == OPT_LEGAL)
      ...
      for (ei = 0; ei < ne; ei++)
	{
      ...
      //      std::cout << cntill << " illegal tets" << endl;
    }
...
void MeshOptimize3d :: SwapImprove (Mesh & mesh, OPTIMIZEGOAL goal)
{
  ...
  /*
    ...
    std::cout << "detected illegal tet1: " << i << endl;
    }
  */	
  ...
  */
  ...
  for (ei = 0; ei < ne; ei++)
      ...
      for (j = 0; j < 6; j++)
	{
	  ...
	  //	  std::cout << "check element " << elemi << endl;
	  ...
	  if ( nsuround == 3 )
	      ...
	      if (goal == OPT_CONFORM && bad2 < 1e4)
		{
		  ...
		  if (faces.Used(face))
		    {
		      // std::cout << "3->2 swap, could improve conformity, bad1 = " << bad1
		      //				 << ", bad2 = " << bad2 << endl;
		      if (bad2 < 1e4)
			bad1 = 2 * bad2;
		    }
		...
		{
		  //		  std::cout << "3->2 " << flush;
		  //		  std::cout << "3->2 conversion" << endl;
		  cnt++;
		  

		  /*
		  std::cout << "3->2 swap, old els = " << endl
	  ...
	  if (nsuround == 4)
	      ...
	      if (swap2 || swap3)
		{
		  // std::cout << "4->4 " << flush;
		  cnt++;
		  //		  std::cout << "4->4 conversion" << "\n";
		  /*
		    std::cout << "bad1 = " << bad1 
		    << " bad2 = " << bad2
		    << " bad3 = " << bad3 << "\n";
		  
		    std::cout << "Points: " << pi1 << " " << pi2 << " " << pi3 
		    << " " << pi4 << " " << pi5 << " " << pi6 << "\n";
		    std::cout << "Elements: " 
		    << hasbothpoints.Get(1) << "  "
		    << hasbothpoints.Get(2) << "  "
		  ...
		  */
		  ...
		  /*
		    ...
		    {
		    for (j1 = 1; j1 <= 4; j1++)
		    std::cout << volelements.Get(hasbothpoints.Get(i1)).PNum(j1)
		    << "  ";
		    std::cout << "\n";
		    }
		    }
		  */
	      ...
	      if (swap2)
		{
		  //		  std::cout << "bad1 = " << bad1 << " bad2 = " << bad2 << "\n";


		  /*
		  std::cout << "4->4 swap A, old els = " << endl
			     << mesh[hasbothpoints[0]] << endl
			     << mesh[hasbothpoints[1]] << endl
			     << mesh[hasbothpoints[2]] << endl
	  ...
	  if (nsuround >= 5) 
	      ...
	      for (k = 0; k < nsuround; k++)
		{
	      ...
	      //  std::cout << "nsuround = " << nsuround << " bad1 = " << bad1 << endl;
	      ...
	      for (l = 0; l < nsuround; l++)
		{
		  ...
		  if (goal == OPT_CONFORM)
		      ...
		      for (k = l+1; k <= nsuround + l - 2; k++)
			{
			  ...
			  if (faces.Used(hi3))
			    {
			      // std::cout << "could improve face conformity, bad1 = " << bad1
		      ...
		      for (k = l+2; k <= nsuround+l-2; k++)
			{
			  if (mesh.BoundaryEdge (suroundpts[l],
			    ...
			    {
			      /*
			      std::cout << "could improve edge conformity, bad1 = " << bad1
				   << ", bad 2 = " << bad2 << ", nottoobad = " << nottoobad << endl;
			      */
			      if (nottoobad)
				confedge = l;
		...
		bestl = confface;
	      ...
	      if (bestl != -1)
		{
		  // std::cout << nsuround << "->" << 2 * (nsuround-2) << " " << flush;
		  cnt++;
		  ...
		  for (k = bestl+1; k <= nsuround + bestl - 2; k++)
		    {
		      ...
		      /*
		      std::cout << nsuround << "-swap, new el,top = "
				 << hel << endl;
		      */
			...
			elementsonnode.Add (hel[k1], mesh.GetNE()-1);
		      ...
		      /*
		      std::cout << nsuround << "-swap, new el,bot = "
				 << hel << endl;
		      */
  ...
  */
  ...
  for (eli1 = 0; eli1 < ne; eli1++)
	...
	continue;
      ...
      // cout << "eli = " << eli1 << endl;
      //      std::cout << "swapimp2, eli = " << eli1 << "; el = " << mesh[eli1] << endl;
  ...
  /*
    ...
    {
    cout << "detected illegal tet, 2" << endl;
    std::cout << "detected illegal tet2: " << i << endl;
    }
  */
  ...
  bad1 = CalcTotalBad (mesh.Points(), mesh.VolumeElements());
  std::cout << "Total badness = " << bad1 << endl;
  std::cout << "swapimprove2 done" << "\n";
  //  std::cout << "Vol = " << CalcVolume (points, volelements) << "\n";
}
