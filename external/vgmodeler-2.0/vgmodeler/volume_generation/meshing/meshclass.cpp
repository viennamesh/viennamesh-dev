...
//     std::cout << ".. in AddPoint .. after append.." << std::endl;
...
//     lock.Lock();
    ...
    //std::cout << "## correct func.. " << std::endl;
//     lock.UnLock();
...
//    clusters -> Update();
  ...
  {
//      int timer = NgProfiler::CreateTimer ("Mesh::FindOpenElements");
//      int timera = NgProfiler::CreateTimer ("Mesh::FindOpenElements A");
//      int timerb = NgProfiler::CreateTimer ("Mesh::FindOpenElements B");
//      int timerc = NgProfiler::CreateTimer ("Mesh::FindOpenElements C");
//      int timerd = NgProfiler::CreateTimer ("Mesh::FindOpenElements D");
//      int timere = NgProfiler::CreateTimer ("Mesh::FindOpenElements E");
				  ...
				  if (i2.I1() == 0)
				    {
				      ...
				      std::cout  << "more elements on face!!!" << endl;
				      std::cout << "el = " << el << endl;
				      std::cout << "hel = " << hel << endl;
				      std::cout << "face = " << i3 << endl;
				      std::cout << "points = " << endl;
				      for (int jj = 1; jj <= 3; jj++)
					std::cout << "p = " << Point(i3.I(jj)) << endl;
				    }
				}
			    }
			  else
...
//     NgProfiler::RegionTimer rege (timere);
  ...
  void Mesh :: FindOpenSegments (int surfnr)
  {
	...
	if (surfnr == 0 || seg.si == surfnr)
	  {
		...
		std::cout << "ERROR: Segment " << seg << " already used" << endl;
	      }
	...
	if (surfnr == 0 || seg.si == surfnr)
	  {
		...
		std::cout << "ERROR: Segment " << seg << " brother not used" << endl;
	      }
	  }
      }
      ...
      for (j = 1; j <= faceht.GetBagSize(i); j++)
	{
	  ...
	  if (data.I1())  // surfnr
		...
		{
		  std::cout << "Problem with open segment: " << seg << endl;
		}
    ...
    std::cout << opensegments.Size() << " open segments found" << endl;
  ...
  void Mesh :: SetLocalH (const Point3d & pmin, const Point3d & pmax, double grading, long threadID)
  {
...
#ifdef FSDEBUG
    std::cout << "..checkpoint SetLocalH before delete lochfunc \t\t\t " << threadID  << std::endl;
#endif  
...
#ifdef FSDEBUG
    std::cout << "..checkpoint SetLocalH after creation lochfunc \t\t\t "<< threadID  << std::endl;
#endif  
  ...
  void Mesh :: CalcLocalH (long threadID) 
  {
#ifdef FSDEBUG
    std::cout << ".. in MesH::CalcLocalH .. "  << threadID << std::endl;
#endif    
...
#ifdef FSDEBUG
	std::cout << "..checkpoint CalcLocalH no lochfunc \t\t\t " << threadID  << std::endl;
#endif

         Point3d pmin, pmax;
         GetBox (pmin, pmax);
         SetLocalH (pmin, pmax, mparam.grading, threadID);
      }

#ifdef FSDEBUG
    std::cout << "..checkpoint CalcLocalH A \t\t\t "  << threadID  << std::endl;
#endif
...
// 		  GetNSE(), " Surface Elements");
		...
		*/
		if (!ident -> UsedSymmetric (el.PNumMod(j),
					     el.PNumMod(j+1)))
		  {
		    ...
		    //		  std::cout << "trigseth, p1,2 = " << el.PNumMod(j) << ", " << el.PNumMod(j+1) 
		    //			     << " h = " << (2 * Dist(p1, p2)) << endl;
		  }
	      }

#ifdef FSDEBUG
	    std::cout << "..checkpoint CalcLocalH A1 \t\t\t " << threadID  << std::endl;
#endif
...
#ifdef FSDEBUG
		std::cout << "..checkpoint CalcLocalH A2 \t\t\t " << threadID  << std::endl;
#endif
	      }
	  }
	else
...
#ifdef FSDEBUG
	    std::cout << "..checkpoint CalcLocalH A3 \t\t\t " << threadID  << std::endl;
#endif
...
#ifdef FSDEBUG
	    std::cout << "..checkpoint CalcLocalH A4 \t\t\t " << threadID  << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint CalcLocalH B \t\t\t " << threadID  << std::endl;
#endif
	...
	*/
    ...
    /*
      ...
      lochfunc->SetH (Center (p1, p2), 2 * Dist (p1, p2));
      std::cout << "set vol h to " << (2 * Dist (p1, p2)) << endl;
...
#ifdef FSDEBUG
    std::cout << ".. leaving MesH::CalcLocalH .. " << threadID << std::endl;
#endif    
  }
  ...
  void Mesh :: CalcLocalHFromPointDistances(void)
  {
	...
	SetLocalH (pmin, pmax, mparam.grading, -1);
      }
	...
	for (j = 1; j <= 3; j++)
	  {
		...
		/*	      
		  std::cout << "pi1,2, 3, 4 = " << i2.I1() << ", " << i2.I2() << ", " << pi3 << ", " << pi4
		  << " p1 = " << Point(i2.I1()) 
		  << ", p2 = " << Point(i2.I2()) 
		  //			 << ", p3 = " << Point(pi3) 
		  //			 << ", p4 = " << Point(pi4) 
  ...
  void Mesh :: Compress ()
  {
    ...
    /*
      std::cout << "volels: " << endl;
      for (i = 1; i <= volelements.Size(); i++)
      {
      for (j = 1; j <= volelements.Get(i).GetNP(); j++)
      std::cout << volelements.Get(i).PNum(j) << " ";
      std::cout << endl;
      }
      std::cout << "np: " << GetNP() << endl;
    */
...
#ifdef FSDEBUG
    std::cout << ".. after volelements .. " << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << ".. after surfaceelems .. " << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << ".. after segments .. " << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << ".. after openelems .. " << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << ".. after lockedpoints .. " << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << " .. checkpoint 1a .. " << points.Size() << " :: " << npi << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << ".. after calcsurfacenode .. " << std::endl;
#endif
	  ...
	  if (cnt)
	    {
	      ...
	      std::cout << "Edge " << i2 << " multiple times in surface mesh" << std::endl;
#endif
	      i2s = i2;
	      i2s.Sort();
	      for (int k = 1; k <= nf; k++)
	...
	for (j = 1; j <= inters.Size(); j++)
	  {
		...
		std::cout << "openelement " << i << " with open element " << inters.Get(j) << endl;
...
// 		cout << "layer2 = " <<  (*this)[tri2[0]].GetLayer() << endl;
		...
		std::cout << "Face1 = " << GetFaceDescriptor(tri.GetIndex()) << endl;
		std::cout << "Face1 = " << GetFaceDescriptor(tri2.GetIndex()) << endl;
  ...
  bool Mesh :: LegalTet2 (Element & el) const
  {
    ...
    for (i = 0; i < 4; i++)
      {
	bool alledges = 1;
	if ( (*this)[el[i]].Type() == SURFACEPOINT)
	  {
	    for (j = 0; j < 4; j++)
	      if (j != i)
		{
    ...
    */
      ...
      if (ltestmode)
	{
      ...
      for (i = 1; i <= 4; i++)
	if ( (*this)[el.PNum(i)].Type() == SURFACEPOINT)
	  for (j = 1; j <= 4; j++)
	    if (j != i)
	      for (k = j+1; k <= 4; k++)
		if (k != i)
		  {
		    ...
		    /*
		      ...
		      if (ltestmode)
		      {
		      std::cout << "i, j, k = " << i << ", " << j << ", " << k << endl;
		      std::cout << "eij = " << boundaryedges->Used(e1) 
...

    {
      ...
      for (i = 1; i <= 4; i++)
	if ( (*this)[el.PNum(i)].Type() == EDGEPOINT)
	  for (j = 1; j <= 4; j++)
	    if (j != i)
	      for (k = j+1; k <= 4; k++)
		if (k != i)
  ...
  {
//      int timer = NgProfiler::CreateTimer ("GetSurfaceElementsOfFace");
//     NgProfiler::RegionTimer reg (timer);
  ...
  int Mesh :: MarkIllegalElements ()
  {
	...
	/*
	  ...
	  std::cout << "legal differs" << endl;
	  std::cout << "elnr = " << i << ", el = " << el
  ...
  {
    int i, j, nv;
    int ne = GetNE();    // RH get number of volume elements
    int nse = GetNSE();  // RH get number of surface elements
	...
	nv = el.GetNV();
	for (j = 0; j < nv; j++)
	  if (el[j] > numvertices)          // RH maximum search
	    numvertices = el[j];
      }
    for (i = 1; i <= nse; i++)
      {
	...
	nv = el.GetNV();
	for (j = 1; j <= nv; j++)
	  if (el.PNum(j) > numvertices)      // RH maximum search
  ...
  /*
    ...
    {
    int i, n2;
    //  std::cout << "connect " << node << " to " << tonode << endl;
...
//       surfelementht->PrintMemInfo (cout);
  ...
  // [RH] extension
  void Mesh::get_elements_on_vertex(int vertex_number, ARRAY<int>& element_container)
  {
    topology->GetVertexElements(vertex_number, element_container);
  }
}
