...
namespace vgmodeler
{
  ...
  void Meshing2 :: AddPoint (const Point3d & p, PointIndex globind, 
			     ...
			     bool pointonsurface)
  {
    //std::cout << "add point " << globind << endl;
    adfront ->AddPoint (p, globind, mgi, pointonsurface);
  }

  // [RH] .. correct datatype
  //
  void Meshing2 :: AddBoundaryElement (INDEX i1, INDEX i2,
				       const PointGeomInfo & gi1, const PointGeomInfo & gi2)
  {
    //    std::cout << "add line " << i1 << " - " << i2 << endl;
  ...
  {
    // [RH] correct data type .. INDEX instead of int..
    //
    ARRAY<INDEX> pindex, lindex;
    ARRAY<INDEX> delpoints, dellines;
	...
	*/
	...
	// [RH] .. final mesh.. step 1.0: restrict local featuer size
	//
	...
	//std::cout << "h for locals: " << 2*hinner << endl;
	

	//std::cout << "locpoints " << locpoints << endl;
	...
	if (found)
	  {
	    ...
	    if (debugflag)
	      std::cout << "define new transformation" << endl;
	    ...
	    //	    std::cout << "3d->2d transformation" << endl;

	    for (int i = 1; i <= locpoints.Size(); i++)
	      {
		// std::cout << "pindex(i) = " << pindex[i-1] << endl;
		TransformToPlain (locpoints.Get(i), 
				  mpgeominfo.Get(i),
				  plainpoints.Elem(i), h, plainzones.Elem(i));
		//		std::cout << mpgeominfo.Get(i).GetPGI(1).u << " " << mpgeominfo.Get(i).GetPGI(1).v << " ";
		//		std::cout << plainpoints.Get(i).X() << " " << plainpoints.Get(i).Y() << endl;
		//std::cout << "transform " << locpoints.Get(i) << " to " << plainpoints.Get(i).X() << " " << plainpoints.Get(i).Y() << endl;
	      }
		...
		if ( (z1 >= 0) != (z2 >= 0))
		  {
			...
			if (!morerisc)
			  {
			    ...
			    double len = v.Length();
			    if (len <= 1e-6)
			      std::cout << "WARNING(js): inner-outer: short vector" << endl;
			    else
			      v /= len;
			...
			//		  std::cout << "inner - outer correction" << endl;
		      }
		    else
		      {
			// remove line
	    ...
	    */  
	    ...
	    /*	  
		 ...
		 clp++;
		 std::cout << "legalpts: " << clp << "/" << plainpoints.Size() << endl; 
		 ...
		 std::cout << "leglines " << lastleg << "/" << oldnl << endl;
	    */
		...
		for (int i = 1; i <= chartboundlines.Size(); i++)
		  {
		    ...
		    //	      std::cout << "line: " << line.I1() << "-" << line.I2() << endl;
		  }
	      }
		...
		if (nlgpi1 <= pindex.Size() && nlgpi2 <= pindex.Size())
		  {
			...
			std::cout << "ERROR: new line exits, val = " << exval << endl;
			found = 0;
	...
	if (found)
	  {
		...
		// [RH] .. final mesh.. step 2.0: add points
		//
		...
		/*
		  ...
		  {
		  std::cout << j << ": " << locpoints.Get(j) << endl;
		  }
		*/
		...
		*/		  
		...
		// ===============================================================
		//
		// [RH] .. final mesh.. step 3.0: add surface elements
		//
		...
		for (int j = 1; j <= locelements.Get(i).GetNP(); j++)
		  {
			...
			std::cout << "illegal point: " << gpi << endl;
		      }
		...
		*/
		...
		std::cout << "success of rule" << rules.Get(rulenr)->Name() << endl;
		std::cout << "trials = " << trials << endl;

		std::cout << "locpoints " << endl;
		for (int i = 1; i <= pindex.Size(); i++)
		  std::cout << adfront->GetGlobalIndex (pindex.Get(i)) << endl;

		std::cout << "old number of lines = " << oldnl << endl;
