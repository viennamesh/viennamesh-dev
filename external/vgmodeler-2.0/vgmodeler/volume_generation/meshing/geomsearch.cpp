...
namespace vgmodeler
{
  ...
  void GeomSearch3d :: Create()
  {
	...
	*/
	...
	// [RH] .. initializiation of min, max values
	//
	...
	// [RH] get min/max extension of frontfaces
	//
	for (i = 1; i <= faces->Size(); i++)
	  {
	    ...
	    // [RH]
	    //std::cout << "i: " << i << "  xxx " << faces->Get(i).Face() << std::endl;
	    
	    MinCoords(minp, minext);   // [RH] .. here, the minext value is assigned
	    MaxCoords(maxp, maxext);   // [RH] ..       the maxext value is assigned
	    midext+=maxp-minp;
	  }
	std::cout << "== minext: "<< minext << "  maxext: " << maxext << std::endl;
	std::cout << "== minp: "<< minp << "  maxp: " << maxp << std::endl;

	// [RH] .. test case with double min/max
	//minext=Point3d(1e-16,1e-16,1e-16);
	//maxext=Point3d(1e16, 1e16, 1e16);
	...
	std::cout << "i1: " << size.i1 << "  i2: " << size.i2 << "  i3: " << size.i3 << std::endl;
	...
	for (iy = sy; iy <= ey; iy++)
	  {
		...
		if (ind < 1 || ind > size.i1 * size.i2 * size.i3)
		  {
		    std::cout <<"## size: "<< size.i1 * size.i2 * size.i3 << std::endl;
		    std::cout << "#ez : "<< ez << std::endl;
		    std::cout << "#sz : "<< sz << std::endl;
		    std::cout << "#sz calc : "<< ((minp.Z()-minext.Z())/elemsize.Z()) << std::endl;
		    std::cout << "#sz calc1 : "<< minp.Z() << std::endl;
		    std::cout << "#sz calc2 : "<< minext.Z() << std::endl;
		    std::cout << "#sz calc3 : "<< elemsize.Z() << std::endl;
		    std::cout << "ix: " << ix << "  iy: " << iy << "  iz-1: " << iz-1 << std::endl;
		    std::cout << "i1: " << size.i1 << "  i2: " << size.i2 << std::endl;
		    std::cout  << "Illegal hash-position: " << ind << " ... " << std::endl;
		    std::cout << "Position: " << ix << "," << iy << "," << iz << endl;
// 		    cerr << "Illegal hash-position: " << ind << " ... " << std::endl;
// 		    cerr << "Position: " << ix << "," << iy << "," << iz << endl;
  ...
  {
    //std::cout << "!!## getlocals .. " << hashcount << std::endl;
    hashcount++;
    ...
    /*
      ...
      {
      std::cout << "n.o.f= " << faces->Size();
      std::cout << ", n.o.lf= " << locfaces.Size();
      std::cout << ", hashf= " << (double)cnt2/(double)faces->Size();
      std::cout << " (" << (double)cnt1/(double)faces->Size();
      std::cout << ", " << (double)cnt3/(double)faces->Size() << ")" << endl;
      }
    */
