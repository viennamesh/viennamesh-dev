	       ...
	       DenseMatrix & l, Vector & d)
{
  ...
  /*
  // Multiply:
  std::cout << "multiplied factors: " << endl;
  for (i = 1; i <= n; i++)
    for (j = 1; j <= n; j++)
      {
	x = 0;
...

{
  ...
  do
	...
	{
	  std::cout << "x = " << x << endl;
	  std::cout << "xneu = " << xneu << endl;
	  std::cout << "f = " << f << endl;
	  std::cout << "g = " << g << endl;
	}
      */

      //      std::cout << "it = " << it << " f = " << f << endl;
      //      if (ifail != 0) break;
	...
	if ( fabs (g.Elem(i)) * max2 (typx.Elem(i), fabs (x.Elem(i))) > hd)
      ...
      //    std::cout << "g = " << g << endl;
      //    std::cout << "a1crit, a3crit = " << int(a1crit) << ", " << int(a3acrit) << endl;
	...
	std::cout << "it =" << setw (5) << it
	<< " f =" << setw (12) << setprecision (5) << f
	<< " |g| =" << setw (12) << setprecision (5) << normg;

	std::cout << " x = (" << setw (12) << setprecision (5) << x.Elem(1);
	for (i = 2; i <= n; i++)
	std::cout << "," << setw (12) << setprecision (5) << x.Elem(i);
	std::cout << ")" << endl;
	*/

      //      std::cout << "it = " << it << " f = " << f << " x = " << x << endl
      //		 << " g = " << g << " p = " << p << endl << endl;

      //      std::cout << "|g| = " << g.L2Norm() << endl;
  ...
  /*
  std::cout << "it = " << it << " g = " << g << " f = " << f 
	     << " fail = " << ifail << endl;
  */
  if (f0 < f || (ifail == 1))
    {
#ifdef DEBUG
      std::cout << "fail, f = " << f << " f0 = " << f0 << endl;
#endif
      f = f0;
      x = x0;
    }

  //  std::cout << "x = " << x << ", x0 = " << x0 << endl;
  return f;
}
