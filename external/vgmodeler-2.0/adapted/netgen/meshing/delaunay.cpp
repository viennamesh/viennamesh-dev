...
namespace netgen
{
  ...
  */
  ...
  void AddDelaunayPoint (PointIndex newpi, const Point3d & newp, 
			 ...
			 IndexSet & insphere, IndexSet & closesphere, int threadID)
  {
    int i, j, k, l;
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint A \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG	
    std::cout << "..checkpoint AddDelaunayPoint B \t\t\t" << threadID << std::endl;
#endif
    ...
    */
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint C \t\t\t" << threadID << std::endl;
#endif
    ...
    //      std::cout << "newels: " << endl;
    ARRAY<Element> newels;

#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint D \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint D1 \t\t\t" << threadID << std::endl;
#endif
	  ...
	  if (!nbind || !insphere.IsIn (nbind))
	      ...
	      if (hval > -1e-12)
		{
		  ...
		  std::cout << "vec to outer, hval = " << hval << endl;
		  std::cout << "v1 x v2 = " << Cross (v1, v2) << endl;
		  std::cout << "facep: "
			     << mesh.Point (face[0]) << " "
			     << mesh.Point (face[1]) << " "
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint E \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint E1 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint F \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint F1 \t\t\t" << threadID << std::endl;
#endif
	...
	if (CalcSphereCenter (&pp[0], pc) )
	  {
	    ...
	    std::cout << "new tet is flat" << endl;
	    for (k = 1; k <= 4; k++)
	      std::cout << newels.Get(j).PNum(k) << " ";
	    std::cout << endl;
	    for (k = 1; k <= 4; k++)
	      std::cout << *pp[k-1] << " ";
	    std::cout << endl;
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint F2 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint F3 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint AddDelaunayPoint G \t\t\t" << threadID << std::endl;
#endif
  }
  ...
  void Delaunay1 (Mesh & mesh, const MeshingParameters & mp, AdFront3 * adfront,
		  ARRAY<DelaunayTet> & tempels,
		  int oldnp, DelaunayTet & startel, Point3d & pmin, Point3d & pmax, int threadID)
  {
    int i, j, k, l;
    const Point3d * pp[4];
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 A \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 B \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 C \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 D \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 E \t\t\t" << threadID << std::endl;
#endif
...
// 	  break;
...
#ifdef FSDEBUG
	std::cout << "..checkpoint Delaunay1 E1 \t\t\t" << threadID << std::endl;
#endif

	AddDelaunayPoint (newpi, newp, tempels, mesh,
			  tettree, meshnb, centers, radi2, 
			  connected, treesearch, freelist, list, insphere, closesphere, threadID);

#ifdef FSDEBUG
	std::cout << "..checkpoint Delaunay1 E2 \t\t\t" << threadID << std::endl;
#endif

      }

#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 F \t\t\t" << threadID << std::endl;
#endif
	...
	{
#ifdef FSDEBUG
	  std::cout << "..checkpoint Delaunay1 F1 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay1 G \t\t\t" << threadID << std::endl;
#endif
  ...
  void Meshing3 :: Delaunay (Mesh & mesh, int domainnr, const MeshingParameters & mp, int threadID)
  {
    int np, ne;
...
//     PushStatus ("Delaunay meshing");
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay A \t\t\t" << threadID << std::endl;
#endif

    Delaunay1 (mesh, mp, adfront, tempels, oldnp, startel, pmin, pmax, threadID);

#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay A1 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay A2 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A3 \t\t\t" << threadID << std::endl;
#endif

      MeshQuality3d (tempmesh);
    
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A4 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A5 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A6 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A7 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A8 \t\t\t" << threadID << std::endl;
#endif
	  
      MeshQuality3d (tempmesh);
    
#ifdef FSDEBUG
      std::cout << "..checkpoint Delaunay A9 \t\t\t" << threadID << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay B" << std::endl;
#endif
...
#ifdef DEBUG
    std::cout << "..checkpoint Delaunay C" << std::endl;
#endif
...
#ifdef FSDEBUG
    std::cout << "..checkpoint Delaunay D" << std::endl;
#endif
    ...
    */
    ...
    /*
      std::cout << "nb elements:" << endl;
      for (i = 1; i <= tempels.Size(); i++)
      {
      std::cout << i << " ";
      for (j = 1; j <= 4; j++)
      std::cout << tempels.Get(i).NB1(j) << " ";
      std::cout << endl;
      }
	...
	*/
    ...
    std::cout << "Check Triangle " << j << ":";
    for (k = 1; k <= 3; k++)
    std::cout << " " << sel.PNum(k);
    for (k = 1; k <= 3; k++)
    std::cout << " " << tempmesh.Point(sel.PNum(k));
    std::cout << endl;

    std::cout << "Check Tet " << i << ":";
    for (k = 1; k <= 4; k++)
    std::cout << " " << el.PNum(k);
    for (k = 1; k <= 4; k++)
    std::cout << " " << tempmesh.Point(el.PNum(k));
    std::cout << endl;
...
#ifdef FSDEBUG
    std::cout << " .. [Delaunay] after compress" << std::endl;
#endif
