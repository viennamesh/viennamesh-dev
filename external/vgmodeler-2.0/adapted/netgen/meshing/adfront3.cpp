...
{
//    int timer_a = NgProfiler::CreateTimer ("Adfront3::RebuildInternal A");
//    int timer_b = NgProfiler::CreateTimer ("Adfront3::RebuildInternal B");
//    int timer_c = NgProfiler::CreateTimer ("Adfront3::RebuildInternal C");
//    int timer_d = NgProfiler::CreateTimer ("Adfront3::RebuildInternal D");
...
int AdFront3 :: GetLocals (int fstind,
			   ...
			   INDEX& facesplit)
{
//    int timer = NgProfiler::CreateTimer ("AdFront3::GetLocals");
	      ...
	      for (j = 1; j <= connectedpairs->EntrySize(pi); j++)
		{
		  ...
		  if (other >= 1 && other <= pindex.Size() && 
		      pindex.Get(other) == oi)
		    {
		      ...
		      // coned.Sort();
		      // std::cout << "connected: " << locpoints.Get(i) << "-" << locpoints.Get(other) << endl;
			  ...
			  const ARRAY<int> * testfaces) const
{
  ...
  //  std::cout << "test ss, p1,p2 = " << lp1 << lp2 << ", inters = " << aprif.Size() << endl;
  //  for (i = 1; i <= faces.Size(); i++)
