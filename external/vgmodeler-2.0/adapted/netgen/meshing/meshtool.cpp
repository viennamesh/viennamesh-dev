...
namespace netgen
{
  ...
  int CheckSurfaceMesh2 (const Mesh & mesh)
  {
   ...
   double TriangleQualityInst (const Point3d & p1, const Point3d & p2,
				     const Point3d & p3)
  {
  ...
  void MeshQuality2d (const Mesh & mesh)
  {
    ...
    std::cout << "Points:           " << mesh.GetNP() << endl;
    std::cout << "Surface Elements: " << mesh.GetNSE() << endl;

    std::cout << endl;
    std::cout << "Elements in qualityclasses:" << endl;
    std::cout.precision(2);
    for (i = 1; i <= ncl; i++)
      {
	std::cout << setw(4) << double (i-1)/ncl << " - "
		   << setw(4) << double (i) / ncl << ": "
		   << incl.Get(i) << endl;
      }
  }
   ...
   double TetElementQuality (const Point3d & p1, const Point3d & p2,
...
#ifdef DEBUG
    std::cout << "Volume elements in qualityclasses:" << endl;
    std::cout.precision(2);
    for (i = 1; i <= ncl; i++)
      {
	std::cout << setw(4) << double (i-1)/ncl << " - "
