...
GradingBox :: GradingBox (const double * ax1, const double * ax2)
{
  ...
  /*
  std::cout << "new box: " << xmid[0] << "-" << xmid[1] << "-" << xmid[2] 
	     << " h = " << (x2[0] - x1[0]) << endl;
  */
...
double LocalH :: GetH (const Point3d & x) const
{
  ...
  while (1)
	...
	{
	  //	  std::cout << "diam = " << (box->x2[0] - box->x1[0])
	  //		     << " h = " << box->hopt << endl;
	  return box->hopt;
	}
    }
			       ...
			       int (*testinner)(const Point3d & p1))
{
  ...
  if (testinner)
    std::cout << "inner = " << root->flags.pinner << " =?= " 
