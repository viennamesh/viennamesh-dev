...
namespace netgen
{
  ...
  ExtrusionFace :: ExtrusionFace(const ARRAY<double> & raw_data)
  {
	...
	profile = new LineSeg<2>(GeomPoint<2>(p[0],1),
				 GeomPoint<2>(p[1],1));
	//std::cout << "appending LineSeg<2> " << p[0] 
	//	   << " to " << p[1] << endl;
      }
    else if(ptype == 3)
      {
	profile = new SplineSeg3<2>(GeomPoint<2>(p[0],1),
				    GeomPoint<2>(p[1],1),
				    GeomPoint<2>(p[2],1));
	//std::cout << "appending SplineSeg<3> "
	//	   << p[0] << " -- " << p[1] << " -- " << p[2] << endl;
      }
    ...
    //std::cout << "read glob_z_direction " << glob_z_direction << endl;
				 ...
				 int & seg, double & t) const
  {
	...
	*/
    ...
    //std::cout << " cutdist " << cutdist << " mindist " << mindist << endl;
    ...
    //std::cout << "point "<< point3d << " candidates: ";
    for(int i=0; i<path->GetNSplines(); i++)
      {
  ...
  double ExtrusionFace :: CalcFunctionValue (const Point<3> & point) const
  {
    ...
    //std::cout << "spline " << dummyi << " t " << dummyd << endl;
  ...
  bool ExtrusionFace :: BoxIntersectsFace(const Box<3> & box) const
  {
    ...
    //std::cout << "box.Center() " << box.Center() << " projected " << center << " diam " << box.Diam() 
  ...
  void ExtrusionFace :: GetRawData(ARRAY<double> & data) const
  {
    ...
    //std::cout << "written raw data " << data << endl;
  }
					  ...
					  ARRAY<int> * const facenums) const
  {
	...
	//std::cout << "intersects " << intersects << " before " << before << " after " << after << endl;
	if(intersects)
	  {
	    if(facenums)
	      {
