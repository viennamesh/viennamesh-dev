...
//     PushStatus ("Find edges");
  ...
  void EdgeCalculation :: CalcEdges1 (double h, Mesh & mesh)
  {
    ...
    //  std::cout << "hsp["<<i<<"] ... " << specpoints[hsp[i]].p << endl;
	...
	for (int i = 0; i < specpoints.Size(); i++)
	  for (int j = 0; j < geometry.GetNTopLevelObjects(); j++)
		...
		{
#ifdef DEVELOP
		  std::cout << "point " << specpoints[i].p << " v " <<specpoints[i].v <<" is in " << tlo->GetSolid()->Name() << endl;
#endif
...
#ifdef DEVELOP
	std::cout << "hsp.Size() " << hsp.Size() << " specpoints.Size() " << specpoints.Size() << endl;
	std::cout << endl << "edge nr " << cntedge+1 << endl;
#endif
	...
	for (int i = 0; i < geometry.identifications.Size() && !pi1; i++)
	  for (int j = checkedcopy; j < startpoints.Size() && !pi1; j++)
	    {
#ifdef DEVELOP
	      std::cout << "checking point " << specpoints[startpoints[j]].p
	      ...
	      
		{
		  ...
		  for (int k = 0; k < hsp.Size() && !pi1; k++)
			...
			{
#ifdef DEVELOP
			  std::cout << "identifyable: " << specpoints[hsp[k]].p << ", v = " << specpoints[hsp[k]].v
...
#ifdef DEVELOP
	std::cout << "start followedge: p1 = " << specpoints[hsp.Get(pi1)].p 
	...
	for (int i = 1; i <= geometry.identifications.Size(); i++)
	  if (geometry.identifications.Get(i)->ShortEdge(specpoints[hsp.Get(pi1)], specpoints[hsp.Get(ep)]))
	    shortedge = 1;
	// std::cout << "shortedge = " << shortedge << endl;
...
#ifdef DEVELOP
	std::cout << "terminal point: p = " << specpoints[hsp.Get(ep)].p 
	...
	for (int i = 1; i <= refedges.Size(); i++)
	  std::cout << " " << refedges.Get(i).si;
	std::cout << endl;
	if (refedgesinv.Size())
	  std::cout << "inv[1] = " << refedgesinv.Get(1) << endl;
#endif
...
// 	for(int i=0; i<hsp.Size(); i++)
// 	  {
// 	    std::cout << "pos2 hsp["<<i<<"] ... " << specpoints[hsp[i]].p << endl;
// 	  }
      }
  }
  ...
  */
	      ...
	      ARRAY<double> & curvelength)
  {
    ...
    std::cout << "geometry.GetSurface(s1) -> LocH (p, 3, 1, h) " << geometry.GetSurface(s1) -> LocH (p, 3, 1, h)
	       ...
	       ARRAY<bool> & refedgesinv)
  {
    ...
    if (debug)
      {
	std::cout << "debug edge !!!" << endl;
	std::cout << "edgepoints = " << edgepoints << endl;
	std::cout << "s1, s2 = " << s1_rep << " - " << s2_rep << endl;
      }
	...
	*testout << "hp = " << hp << endl;
	std::cout << "locsol: " << endl;
	if (locsol) locsol->Printstd::cout;
	std::cout << endl;
	...
	if (debug)
	  std::cout << "edge of tlo " << i << ", has " << locsurfind.Size() << " faces." << endl;
	...
	if (debug)
	  std::cout << locsurfind.Size() << " faces on hp" << endl;
	...
	for (j = 0; j < locsurfind.Size(); j++)
	  {      
	    ...
	    if (debug)
	      {
		std::cout << "face " << locsurfind[j] << ", rep = " << rlsi 
			   << " has (t*n) = " << (t*n) << endl;
		std::cout << "n = " << n << endl;
	      }
		...
		if (debug)
		  {
		    std::cout << "onface(" << hp << ", " << m << ")= " << flush;
		    std::cout << locsol->OnFace (hp, m, eps) << flush;
		    std::cout << " n " << n << flush;
		    std::cout << " vec2in = "
	...
	for(int j=i+1; !todelete.Test(i) && j<refedges.Size(); j++)
	  {
	    ...
	    if(refedges[i].si == refedges[j].si &&
	       ...
	       refedges[i].surfnr1 == refedges[j].surfnr1 &&
	       refedges[i].surfnr2 == refedges[j].surfnr2 
	       // && refedgesinv[i] == refedgesinv[j] // JS, 20060802
	       )
	      {
		if(debug)
		  std::cout << "equal segments: " << refedges[i] << " pri " << edges_priority[i] 
	     ...
	     Mesh & mesh)
  {
    ...
    ne = int (len + 0.5);
    if (ne == 0) ne = 1;
    if (Dist2 (edgepoints.Get(1), edgepoints.Last()) < 1e-8*geometry.MaxSize() && 
	ne <= 6) 
      ne = 6;
    corr = len / ne;
    ...
    */
	...
	// std::cout << "test1, store point " << lastpi << ", p = " << p << endl;
      }
	...
	if (thispi == -1)
	  {
	    ...
	    // std::cout << "test2, store point " << thispi << ", p = " << np << endl;
	  }
	...
	for (k = 1; k <= refedges.Size(); k++)
	  {
	    ...
	    mesh.AddSegment (seg);
	    //std::cout << "add seg " << mesh[seg.p1] << "-" << mesh[seg.p2] << endl;
	    //std::cout << "refedge " << k << " surf1 " << seg.surfnr1 << " surf2 " << seg.surfnr2 << " inv " << refedgesinv.Get(k) << endl;
		  ...
		  Mesh & mesh)
  {
    ...
    */
	...
	// std::cout << "test3, store point " << pi1 << ", p = " << p << endl;
      }
	...
	// std::cout << "test4, store point " << pi2 << ", p = " << p << endl;
      }
    ...
    */
    ...
    // std::cout << "short edge " << pi1 << " - " << pi2 << endl;
	...
	if (k == 1) seg.seginfo = (refedgesinv.Get(k)) ? 2 : 1;
	mesh.AddSegment (seg);
	//	  std::cout << "add seg " << seg.p1 << "-" << seg.p2 << endl;
      }
  }
	    ...
	    Mesh & mesh)
  {
...
#ifdef DEVELOP
	std::cout << "adding identification " << mesh[frompi] << "; " << mesh[topi]
		   << " (id " << copyedgeidentification <<")" << endl;
#endif


	/*
	  std::cout << "Add Identification from CopyEdge, p1 = " 
	...
	*/
	...
	//std::cout << "copy edge, pts = " << npi1 << " - " << npi2 << endl;
	...
	for (k = 1; k <= refedges.Size(); k++)
	  {
	    ...
	    if (k == 1) seg.seginfo = refedgesinv.Get(k) ? 2 : 1;
	    mesh.AddSegment (seg);
	    //	  std::cout << "copy seg " << seg.p1 << "-" << seg.p2 << endl;
#ifdef DEVELOP

	    std::cout << "copy seg, face = " << seg.si << ": " 
...
#ifdef DEVELOP      
	std::cout << seg.surfnr1 << ", " << seg.surfnr2 << ", si = " << seg.si << endl;
#endif
...
#ifdef DEVELOP
		std::cout << "Add segment at smooth surface " << i;
		if (i != classrep) std::cout << ", classrep = " << classrep;
		std::cout << ": "
