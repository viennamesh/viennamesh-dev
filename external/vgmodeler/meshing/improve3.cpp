					   ...
					   const ARRAY< ARRAY<int,PointIndex::BASE>* > * idmaps)
{
  ...
  for (ei = 0; ei < ne; ei++)
      ...
      for (j = 0; !swapped && j < 6; j++)
	{
	  ...
	  bool found = false;
	  for(k=0; !found && k<used_idmaps->Size(); k++)
	    {
	  ...
	  for (k = 0; k < elementsonnode[pi1].Size(); k++)
		...
		{ 
		  ...
		  if(elem.GetIndex() == mattype)
		    ...
		    {
		      // only once
		      for (l = 0; l < hasbothpointsother.Size(); l++)
	  ...
	  if(periodic)
	    {
	      for (k = 0; k < elementsonnode[pi1other].Size(); k++)
		{
		  bool has1 = false, has2 = false;
	  ...
	  for(k = 0; k < surfaceelementsonnode[pi1].Size(); k++)
	    {
	      bool has1 = false, has2 = false;
	  ...
	  if(periodic)
	    {
	      for(k = 0; k < surfaceelementsonnode[pi1other].Size(); k++)
		{
		  bool has1 = false, has2 = false;
	  ...
	  for(l=0; l<mesh[sel1].GetNP(); l++)
	    if(mesh[sel1][l] != pi1 && mesh[sel1][l] != pi2)
	      sp1 = mesh[sel1][l];
	  for(l=0; l<mesh[sel2].GetNP(); l++)
	  ...
	  if(periodic)
	    {
	      ...
	      bool change = false;
	      for(l=0; !change && l<mesh[sel1other].GetNP(); l++)
	  ...
	  bool puretet = true;
	  for (k = 0; puretet && k < hasbothpoints.Size(); k++)
	    if (mesh[hasbothpoints[k]].GetType () != TET)
	      puretet = false;
	  for (k = 0; puretet && k < hasbothpointsother.Size(); k++)
	  ...
	  for(int i=0; i<nsuround; i++)
	    {
	      bool done = false;
	      for(int j=i; !done && j<hasbothpoints.Size(); j++)
		{
		  for(int k=0; !done && k<4; k++)
		    if(mesh[hasbothpoints[j]][k] == outerpoints[i])
		      {
			done = true;
			for(int l=0; l<4; l++)
			  if(mesh[hasbothpoints[j]][l] != pi1 &&
			     mesh[hasbothpoints[j]][l] != pi2 &&
			     mesh[hasbothpoints[j]][l] != outerpoints[i])
			    outerpoints[i+1] = mesh[hasbothpoints[j]][l];
		      }
		  if(done)
		    {
		      ElementIndex aux = hasbothpoints[i];
		      hasbothpoints[i] = hasbothpoints[j];
		      hasbothpoints[j] = aux;
		    }
		}
	    }
	  ...
	  for(int i=0; i<nsuroundother; i++)
	    {
	      bool done = false;
	      for(int j=i; !done && j<hasbothpointsother.Size(); j++)
		{
		  for(int k=0; !done && k<4; k++)
		    if(mesh[hasbothpointsother[j]][k] == outerpointsother[i])
		      {
			done = true;
			for(int l=0; l<4; l++)
			  if(mesh[hasbothpointsother[j]][l] != pi1other &&
			     mesh[hasbothpointsother[j]][l] != pi2other &&
			     mesh[hasbothpointsother[j]][l] != outerpointsother[i])
			    outerpointsother[i+1] = mesh[hasbothpointsother[j]][l];
		      }
		  if(done)
		    {
		      ElementIndex aux = hasbothpointsother[i];
		      hasbothpointsother[i] = hasbothpointsother[j];
		      hasbothpointsother[j] = aux;
		    }
		}
	    }
	  ...
	  for(int i=0; i<startpoints; i++)
	    {
	      ...
	      for(int j=0; j<nsuround-1; j++)
		{
		  (*newelts[i])[2*j] = new Element(TET);
		  (*newelts[i])[2*j+1] = new Element(TET);
		  Element & newel1 = *((*newelts[i])[2*j]);
		  Element & newel2 = *((*newelts[i])[2*j+1]);

		  newel1[0] = pi1;
		  newel1[1] = outerpoints[i];
		  newel1[2] = outerpoints[(i+j+1)%outerpoints.Size()];
		  newel1[3] = outerpoints[(i+j+2)%outerpoints.Size()];

		  newel2[0] = pi2;
		  newel2[1] = outerpoints[i];
		  newel2[2] = outerpoints[(i+j+2)%outerpoints.Size()];
		  newel2[3] = outerpoints[(i+j+1)%outerpoints.Size()];
	      ...
	      for(int j = 0; wrongorientation && j<newelts[i]->Size(); j++)
		wrongorientation = wrongorientation && WrongOrientation(mesh.Points(), *(*newelts[i])[j]);
	      
	      currbad = 0;

	      for(int j=0; j<newelts[i]->Size(); j++)
		{
		  if(wrongorientation)
		    Swap((*(*newelts[i])[j])[2],(*(*newelts[i])[j])[3]);


		  // not two new faces on same surface
		  ARRAY<int> face_index;
		  for(int k = 0; k<surfaceindicesonnode[(*(*newelts[i])[j])[0]].Size(); k++)
		    face_index.Append(surfaceindicesonnode[(*(*newelts[i])[j])[0]][k]);
		  ...
		  for(int k=1; k<4; k++)
			...
			{
			  if(face_index[l] != -1 && 
			     !(surfaceindicesonnode[(*(*newelts[i])[j])[k]].Contains(face_index[l])))
			    face_index[l] = -1;
			}
		  ...
		  currbad += CalcBad(mesh.Points(),*(*newelts[i])[j],h);
	  ...
	  for(int i=0; i<startpointsother; i++)
	    {
	      neweltsother[i] = new ARRAY <Element*>(2*(nsuroundother-1));
	      
	      for(int j=0; j<nsuroundother-1; j++)
		{
		  (*neweltsother[i])[2*j] = new Element(TET);
		  (*neweltsother[i])[2*j+1] = new Element(TET);
		  Element & newel1 = *((*neweltsother[i])[2*j]);
		  Element & newel2 = *((*neweltsother[i])[2*j+1]);

		  newel1[0] = pi1other;
		  newel1[1] = outerpointsother[i];
		  newel1[2] = outerpointsother[(i+j+1)%outerpointsother.Size()];
		  newel1[3] = outerpointsother[(i+j+2)%outerpointsother.Size()];

		  newel2[0] = pi2other;
		  newel2[1] = outerpointsother[i];
		  newel2[2] = outerpointsother[(i+j+2)%outerpointsother.Size()];
		  newel2[3] = outerpointsother[(i+j+1)%outerpointsother.Size()];
	      ...
	      bool wrongorientation = true;
	      for(int j = 0; wrongorientation && j<neweltsother[i]->Size(); j++)
		wrongorientation = wrongorientation && WrongOrientation(mesh.Points(), *(*neweltsother[i])[j]);
	      
	      currbad = 0;

	      for(int j=0; j<neweltsother[i]->Size(); j++)
		{
		  if(wrongorientation)
		    Swap((*(*neweltsother[i])[j])[2],(*(*neweltsother[i])[j])[3]);

		  currbad += CalcBad(mesh.Points(),*(*neweltsother[i])[j],h);
		}  
