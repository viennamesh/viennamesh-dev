...
#include <boost/spirit/phoenix/stl/algorithm/detail/has_find.hpp>
#include <boost/spirit/phoenix/stl/algorithm/detail/has_lower_bound.hpp>
#include <boost/spirit/phoenix/stl/algorithm/detail/has_upper_bound.hpp>
#include <boost/spirit/phoenix/stl/algorithm/detail/has_equal_range.hpp>
...
namespace impl
{
    ...
    struct find_end
    {
        ...
        typename apply<R, R2>::type operator()(R& r, R2& r2) const
        {
            return std::find_end(
                detail::begin_(r)
                , detail::end_(r)
        ...
        typename apply<R, R2, P>::type operator()(R& r, R2& r2, P p) const
        {
            return std::find_end(
                detail::begin_(r)
                , detail::end_(r)
    ...
    {
        template<class R, class T>
        struct apply : range_difference<R>
        {};
    ...
    struct lower_bound 
    {
        ...
        template<class R, class T>
        typename apply<R, T>::type execute(R& r, T const& val, mpl::true_) const
        {
            return r.lower_bound(val);
        }

        template<class R, class T>
        typename apply<R, T>::type execute(R& r, T const& val, mpl::false_) const
        {
            return std::lower_bound(detail::begin_(r), detail::end_(r), val);
        }

        template<class R, class T>
        typename apply<R, T>::type operator()(R& r, T const& val) const
        {
            return execute(r, val, has_lower_bound<R>());
        }

        template<class R, class T, class C>
        typename apply<R, T>::type operator()(R& r, T const& val, C c) const
        {
    ...
    struct mismatch
    {
        ...
        template<class R, class I>
        typename apply<R, I>::type operator()(R& r, I i) const
        {
            return std::mismatch(detail::begin_(r), detail::end_(r), i);
        }

        template<class R, class I, class P>
        typename apply<R, I, P>::type operator()(R& r, I i, P p) const
        {
    ...
    struct lexicographical_compare
    {
        ...
        typename apply<R1, R2>::type operator()(R1& r1, R2& r2, P p) const
        {
            return std::lexicographical_compare(
                detail::begin_(r1), detail::end_(r1)
                , detail::begin_(r2), detail::end_(r2)
