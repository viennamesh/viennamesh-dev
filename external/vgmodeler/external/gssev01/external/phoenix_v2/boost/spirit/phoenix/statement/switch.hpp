...
#include <boost/spirit/phoenix/statement/detail/switch_eval.hpp>
#include <boost/spirit/phoenix/statement/detail/switch.hpp>
...
namespace boost { namespace phoenix
{
    ...
    struct switch_gen
    {
        ...
        operator[](Cases const& cases) const
        {
            ...
            switch_composite;
            return switch_composite::apply(cond, cases);
        }

        template <typename D, typename A>
        actor<typename detail::
            switch_composite<Cond, fusion::tuple<actor<D> > >::type>
        operator[](switch_case_base<D, A> const& case_) const
        {
            typedef typename
                detail::switch_composite<Cond, fusion::tuple<actor<D> > >
            switch_composite;
            return switch_composite::apply(cond,
                fusion::tuple<actor<D> >(static_cast<D const&>(case_)));
        }
    ...
    {
        return detail::compose_case_a<D0, D1>::apply(
