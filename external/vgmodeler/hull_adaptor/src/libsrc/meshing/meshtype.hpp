...
public:
  ...
  PointGeomInfo(int value) 
  {
    trignum = value;
  }
  
  PointGeomInfo operator+ (int value)
  {
    return trignum + value;
  }
};
...
{ 
  ...
  { Point<3>::operator= (ap); layer=0; singular=0; 
#ifdef PARALLEL
    isghost = 0;
#endif
  }
...
public:
  ...
  /// surface decoding index - [FS] used during surface meshing; it stores the 
  //  information which face the surfaceelement belongs to
  int si;          
