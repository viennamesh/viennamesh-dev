...
#include "classifyhpel.hpp"  
		     ...
		     Refinement * ref, double fac1) 
  {
    ...
    std::cout << " in HP-REFINEMENT with fac1 " << fac1 <<  endl; 
   
    if(0.001 > fac1 && 0.33 < fac1) 
      { 
	fac1=0.125; 
	cout << "Partition for hp-Refinment not sensible -> set to 0.125" << endl; 
      } 
	...
	while (hprs->splitedges[j][0])
	  {
		...
		Point3d np; 
		for( int l=1;l<=3;l++)
		  np.X(l) = (1-fac1)*mesh.Point(i2.I1()).X(l) 
		    + fac1 * mesh.Point(i2.I2()).X(l); 
	...
	if (hprs->splitfaces)
	  while (hprs->splitfaces[j][0])
	      ...
	      if (!newfacepts.Used (i3))
		{
		  Point3d np; 
		  	for( int l=1;l<=3;l++)
			  np.X(l) = (1-2*fac1)*mesh.Point(i3.I1()).X(l) 
			    + fac1*mesh.Point(i3.I2()).X(l)  + fac1*mesh.Point(i3.I3()).X(l);  
		  int npi = mesh.AddPoint (np);
    ...
    for (int i = 0; i < oldelsize; i++)
      {
	std::cout << "refine element " << i << endl;
	  ...
	  while (hprs->splitelements[j][0])
	    {
	      ...
	      Point3d np; 
	      	for( int l=1;l<=3;l++)
		  np.X(l) = (1-3*fac1)* mesh.Point(el.pnums[hprs->splitelements[j][0]-1]).X(l) 
		    + fac1* mesh.Point(el.pnums[hprs->splitelements[j][1]-1]).X(l)
		    + fac1* mesh.Point(el.pnums[hprs->splitelements[j][2]-1]).X(l)
		    + fac1* mesh.Point(el.pnums[hprs->splitelements[j][3]-1]).X(l); 
	...
	while (hprs->neweltypes[j])
	  {
	    ...
	    /*
	    std::cout  << " newel pnums " ; 
	    for (int k = 0; k < newel.np; k++)  
	      std::cout  << newel.pnums[k] << "\t"; 
	    std::cout << endl; 
	    */
	  ...
	  if (common)
	    {
	      ...
	      cout << " Degenerate Hex found " << endl; 
              std::cout << " Degenerate Hex found " << endl; 
	      HPRefElement el = elements[i];
	      HPRefElement newel = el;
		...
		for(int k=0;k<12;k++) 
		  { 
			...
			std::cout << "DEGHEX TRIG :: newpnums " << newel.pnums[0] << "\t"  << newel.pnums[1] << "\t"  << newel.pnums[2] << endl;  
  ...
  void CalcStatistics (ARRAY<HPRefElement> & elements)
  {
    ...
    for (p = 1; p <= 20; p++)
      {
	memcost = (ntet + nprism + nhex) * pow (_cast<double>(p), 6.0);
	cpucost = (ntet + nprism + nhex) * pow (_cast<double>(p), 9.0);
	  ...
	  case HP_TET_1E_1VA:
	    {
	      ...
	      std::cout << "p1 = " << p1 << ", P1^6 = " << pow (_cast<double>(p1), 6.0)
			 << " (p1-3)^6 = " << pow ( _cast<double>(max2(p1-3, 0)), 6.0) 
			 << " p1^3 = " << pow ( _cast<double>(p1), 3.0) 
			 << " (p1-3)^3 = " << pow ( _cast<double>(p1-3), 3.0) 
			 << " [p1^3-(p1-3)^3]^2 = " << sqr (pow (_cast<double>(p1),3.0) - pow ( _cast<double>(p1-3), 3.0))
			 << endl;

	      p1 /= 2 +1;
	      memcosttet += pow (_cast<double>(p1), 6.0);
	      memcostsctet += pow (_cast<double>(p1), 6.0) - pow ( _cast<double>(max2(p1-3, 1)), 6.0);
	      cpucosttet += pow (_cast<double>(p1), 9.0);
	      break;
	    }
	  case HP_PRISM:
	  case HP_PRISM_SINGEDGE:
	    {
	      int p1 = maxlevel - el.level + 1;
	      p1 /= 2 +1;
	      memcostprism += pow (_cast<double>(p1), 6.0);
	      memcostscprism += pow (_cast<double>(p1), 6.0) - pow ( _cast<double>(max2(p1-3, 1)), 6.0);
	      cpucostprism += pow (_cast<double>(p1), 9.0);
	      break;
	    }
	  case HP_HEX:
	    {	
	      ...
	      p1 /= 2 +1;
	      p2 /= 2 +1;
	      memcosthex += pow (_cast<double>(p1), 4.0) * pow (_cast<double>(p2), 2.0);
	      memcostschex += pow (_cast<double>(p1), 6.0) - pow ( _cast<double>(max2(p1-2, 0)), 6.0);
	      cpucosthex += pow (_cast<double>(p1), 6.0) * pow (_cast<double>(p2), 3.0);
	      break;
	    }
	  default:
	    ;
  ...
  void HPRefinement (Mesh & mesh, Refinement * ref, int levels, double fac1, bool setorders)
  {
    ...
    *mesh.coarsemesh = mesh;
	...
	for(ElementIndex i=0;i<mesh.GetNE(); i++) 
	  { 
	    ...
	    int ref[3];  
	    for(int j=0;j<3;j++) 
	      ref[j] = int(max(double(floor(log(dist[ord_dir[j]]/sqrt(2.))/log(fac1))),0.)); 	
	    
	    // cout << " ref " << ref[0] << "\t" << ref[1] << "\t" << ref[2] << endl; 
	    // cout << " order " << act_ref +1 - ref[0] << "\t" << act_ref +1 - ref[1] << "\t" << act_ref +1 - ref[2] << endl; 
	   	      
	    if(setorders)
	      mesh[i].SetOrder(act_ref+1-ref[0],act_ref+1-ref[1],act_ref+1-ref[2]); 
	  }
	for(SurfaceElementIndex i=0;i<mesh.GetNSE(); i++) 
	  { 
	    Element2d el = mesh[i] ;
	    ...
	    if(setorders)
	      ...
	      // cout << " ref " << ref[0] << "\t" << ref[1] << endl; 
	      // cout << " order " << act_ref +1 - ref[0] << "\t" << act_ref +1 - ref[1] << endl; 
	  }
      }
  }
...
{ 
  ...
  if (mesh.GetDimension() == 3)
	...
	*/
	...
	std::cout << "cornerpoint = " << endl << cornerpoint << endl;
	std::cout << "edgepoint = " << endl << edgepoint << endl;
	...
	std::cout << "singular faces = " << faces << endl;
	std::cout << "singular faces_edges = " << face_edges << endl;
      }
    else
      {
	// 2D case
	...
	for (int i = 1; i <= mesh.GetNSeg(); i++)
	  {
		...
		if (seg.singedge_left * levels >= act_ref)
		  {
		    ...
		    edgepoint.Set(i2.I1());
		    edgepoint.Set(i2.I2());
		    std::cout << " singleft " << endl;  
		    std::cout << " mesh.LineSegment(i).domout " << mesh.LineSegment(i).domout << endl;      
		    std::cout << " mesh.LineSegment(i).domin " << mesh.LineSegment(i).domin << endl;      
	    ...
	    // std::cout << "seg = " << ind << ", " << seg.p1 << "-" << seg.p2 << endl;
	...
	std::cout << "2d sing edges: " << endl << edges << endl;
	std::cout << "2d cornerpoints: " << endl << cornerpoint << endl
  ...
  bool ClassifyHPElements (Mesh & mesh, ARRAY<HPRefElement> & elements, int & act_ref, int & levels)
  {
    ...
    std::cout << "edgepoint_dom = " << endl << edgepoint_dom << endl;

    for( int i = 0; i<elements.Size(); i++) 
      {
	std::cout << "classify element " << i << endl;
	...
	if (!Get_HPRef_Struct (hpel.type)) 
	  {
	    std::cout << "hp-element-type " << hpel.type << " not implemented   " << endl;
	    std::cout << " elType " << hprs->geom << endl; 
