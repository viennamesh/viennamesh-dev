...
//    int timer = NgProfiler::CreateTimer ("EdgeSwapping 2D");
//   NgProfiler::RegionTimer reg1 (timer);
  ...
  double horder;
  double loch;
   const double minangle[] = { 0, 1.481, 2.565, 3.627, 4.683, 5.736, 7, 9 };
...
//    int timer = NgProfiler::CreateTimer ("Combineimprove 2D");
  ...
  for (i = 0; i < seia.Size(); i++)
      ...
      for (j = 0; j < 3; j++)
	{
	  ...
	  */
	  ...
	  if (debugflag)
	    {
	      std::cout << "Combineimprove, face = " << faceindex 
	  ...
	  */
	  ...
	  if (debugflag)
	    {
	      std::cout << "bad1 = " << bad1 << ", bad2 = " << bad2 << endl;
	    }
	  ...
	  if (should)
		...
		if ((*el1p)[l] == pi1)
	      ...
	      // std::cout << "Connect point " << pi2 << " to " << pi1 << "\n";
	      for (k = 0; k < elementsonnode[pi2].Size(); k++)
		{
...
void MeshOptimize2d :: CheckMeshApproximation (Mesh & mesh)
{
  ...
  std::cout << "Check Surface Approxiamtion" << endl;
  std::cout << "Check Surface Approxiamtion" << endl;
