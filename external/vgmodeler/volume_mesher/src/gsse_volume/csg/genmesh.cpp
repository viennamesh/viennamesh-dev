...
#define TCL_ERROR 1
   ...
   void FindPoints (CSGeometry & geom, Mesh & mesh)
  {
    ...
    std::cout << specpoints.Size() << " special points:" << endl;
    for (int i = 0; i < specpoints.Size(); i++)
      specpoints[i].Print std::cout;
   ...
   void FindEdges (CSGeometry & geom, Mesh & mesh, const bool setmeshsize = false)
  {
	...
	//std::cout << "change from " << mesh.LineSegment(i);
	mesh.LineSegment(i).si = ok;
	//std::cout << " to " << mesh.LineSegment(i).si << endl;
      }
      ...
      for (SegmentIndex si = 0; si < mesh.GetNSeg(); si++)
	{
	  ...
	  for (int j = 0; j < loc.Size(); j++)
		  ...
		  Abs (rhs - mat*sol) < 1e-6)
		{
		  ...
		  std::cout << "found intersection at " << ip << endl;
		  std::cout << "sol = " << sol << endl;
		  std::cout << "res = " << (rhs - mat*sol) << endl;
		  std::cout << "segs = " << pi1 << " - " << pi2 << endl;
		  std::cout << "and = " << pj1 << " - " << pj2 << endl << endl;
		}
	    }
	}  
  }
   ...
   void MeshSurface (CSGeometry & geom, Mesh & mesh)
  {
    const char * savetask = multithread.task;
	...
	std::cout << "Surface " << k << endl;
	std::cout << "Face Descriptor: " << fd << endl;
	PrintMessage (1, "Surface ", k, " / ", mesh.GetNFD());

	int oldnf = mesh.GetNSE();
      

	// [RH] .. part I: calculate surface out of csg representation
	//
	const Surface * surf =
	...
	// [RH]  part II: ... class Meshing2Surfaces : public Meshing2
	//
	...
	for (PointIndex pi = PointIndex::BASE; pi < noldp+PointIndex::BASE; pi++)
	  { 
	    ...
	    // [RH] .. step 2.1: surface mesh: add point
	    //
	    meshing.AddPoint (mesh[pi], pi, NULL,
			      surf->PointOnSurface(mesh[pi]));
	  }
	  ...
	  if (mesh[si].si == k)
	    {
	      ...
	      std::cout << "appending segment " << mesh[si] << endl;
	      //<< " from " << mesh[mesh[si][0]]
	      //	 << " to " <<mesh[mesh[si][1]]<< endl;
	    }

	std::cout << "num-segments " << segments.Size() << endl;
	...
	for (int si = 0; si < segments.Size(); si++)
	  {
	    ...
	    // [RH] .. step 2.2: surface mesh: add boundary element
	    //
	...
	std::cout << "domin = " << fd.DomainIn() << ", domout = " << fd.DomainOut()
	...
	// [RH] .. step 3: generate AF surface mesh..   .. 
	//
	//      creates the final surface mesh within the .. mesh.. container
	//
	MESHING2_RESULT res =
	...
	// [RH] .. step 4: additional .. optimize surface..   .. 
	//
	//
	if (segments.Size())   
	  { 
  ...
  // [RH] .. final step I:: generate all different types of meshes.. surface mesh, quality, volume mesh. 
  //
  //
