...
//         }
    ...
    bool mesh_writer::generic_run_nonsegmented( const_parameter_handle const & mesh, std::string const & filename, FileType file_type )
    {
      ...
      if (mesh->is_type<FullMeshType>())
        return write<FullMeshType>( dynamic_handle_cast<const FullMeshType>(mesh), filename, file_type );

      // thin mesh
      if (mesh->is_type<ThinMeshType>())
        return write<ThinMeshType>( dynamic_handle_cast<const ThinMeshType>(mesh), filename, file_type );

      typename result_of::const_parameter_handle<ThinMeshType>::type tmp = mesh->get_converted<ThinMeshType>();
      if (tmp)
        return write<ThinMeshType>(tmp, filename, file_type);
      else
        return false;
    }
    ...
    template<typename TagT, int DimensionV>
    bool mesh_writer::generic_run_segmented( const_parameter_handle const & mesh, std::string const & filename, FileType file_type )
    {
      typedef typename viennamesh::result_of::full_config<TagT, DimensionV>::type FullConfigType;
      typedef typename viennamesh::result_of::thin_config<TagT, DimensionV>::type ThinConfigType;
      ...
      if (mesh->is_type< viennagrid::segmented_mesh<FullMeshType, FullSegmentationOfFullMeshType> >())
        return write<FullMeshType, FullSegmentationOfFullMeshType>(dynamic_handle_cast< const viennagrid::segmented_mesh<FullMeshType, FullSegmentationOfFullMeshType> >(mesh), filename, file_type);
      ...
      if (mesh->is_type< viennagrid::segmented_mesh<FullMeshType, CellOnlySegmentationOfFullMeshType> >())
        return write<FullMeshType, CellOnlySegmentationOfFullMeshType>(dynamic_handle_cast< const viennagrid::segmented_mesh<FullMeshType, CellOnlySegmentationOfFullMeshType> >(mesh), filename, file_type);

      // thin mesh, full segmentation
      if (mesh->is_type< viennagrid::segmented_mesh<ThinMeshType, FullSegmentationOfThinMeshType> >())
        return write<ThinMeshType, FullSegmentationOfThinMeshType>(dynamic_handle_cast< const viennagrid::segmented_mesh<ThinMeshType, FullSegmentationOfThinMeshType> >(mesh), filename, file_type);

      // thin mesh, full segmentation
      if (mesh->is_type< viennagrid::segmented_mesh<ThinMeshType, CellOnlySegmentationOfThinMeshType> >())
        return write<ThinMeshType, CellOnlySegmentationOfThinMeshType>(dynamic_handle_cast< const viennagrid::segmented_mesh<ThinMeshType, CellOnlySegmentationOfThinMeshType> >(mesh), filename, file_type);
      ...
      if (tmp)
        return write<ThinMeshType, CellOnlySegmentationOfThinMeshType>(tmp, filename, file_type);
      else
        return false;
    }
    ...
    template<typename TagT, int DimensionV>
    bool mesh_writer::generic_run( const_parameter_handle const & mesh, std::string const & filename, FileType file_type )
    {
      bool is_segmented = lexical_cast< stringtools::boolalpha_bool >(mesh->get_property("is_segmented").first);

      if ( is_segmented )
        return generic_run_segmented<TagT, DimensionV>( mesh, filename, file_type );
      else
        return generic_run_nonsegmented<TagT, DimensionV>( mesh, filename, file_type );
    }
    ...
    bool mesh_writer::run_impl()
    {
      ...
      if ( cell_type == "1-simplex" && geometric_dimension == 1 )
        return generic_run<viennagrid::line_tag, 1>( mesh, filename(), file_type );

      if ( cell_type == "1-simplex" && geometric_dimension == 2 )
        return generic_run<viennagrid::line_tag, 2>( mesh, filename(), file_type );

      if ( cell_type == "1-simplex" && geometric_dimension == 3 )
        return generic_run<viennagrid::line_tag, 3>( mesh, filename(), file_type );



      if ( cell_type == "2-simplex" && geometric_dimension == 2 )
        return generic_run<viennagrid::triangle_tag, 2>( mesh, filename(), file_type );

      if ( cell_type == "2-simplex" && geometric_dimension == 3 )
        return generic_run<viennagrid::triangle_tag, 3>( mesh, filename(), file_type );



      if ( cell_type == "quadrilateral" && geometric_dimension == 2 )
        return generic_run<viennagrid::quadrilateral_tag, 2>( mesh, filename(), file_type );

      if ( cell_type == "quadrilateral" && geometric_dimension == 3 )
        return generic_run<viennagrid::quadrilateral_tag, 3>( mesh, filename(), file_type );



      if ( cell_type == "3-simplex" && geometric_dimension == 3 )
        return generic_run<viennagrid::tetrahedron_tag, 3>( mesh, filename(), file_type );



      if ( cell_type == "hexahedron" && geometric_dimension == 3 )
        return generic_run<viennagrid::hexahedron_tag, 3>( mesh, filename(), file_type );
