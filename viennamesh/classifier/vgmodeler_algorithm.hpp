...
#ifndef VIENNAMESH_CLASSIFIER_VGMODELER_ALGORITHM_HPP
#define VIENNAMESH_CLASSIFIER_VGMODELER_ALGORITHM_HPP

#include "histogram_latex.hpp"
...
public:
  ...
  void classify()
  {
   ...
   if (RatioBOOL)
   {
      ResultName = viennamesh::vgmodeler::keys::round;   // simplex
      type = TTRound;
   }
   else 
   {
      if (DaggerBOOL)
      {
         ...
         {
            ResultName = viennamesh::vgmodeler::keys::wedge;    // simplex
            type = TTWedge;
         }
         else if (slat>needle)
         {
            ResultName = viennamesh::vgmodeler::keys::slat;    // simplex
            type = TTSlat;
         }
         else
         {
            ResultName = viennamesh::vgmodeler::keys::needle;  // simplex
            type = TTNeedle;
         }
      }
      else
      {
         ...
         {
            ResultName = viennamesh::vgmodeler::keys::spindle;    // simplex
            type = TTSpindle;
         }
         else if ((spade>cap)&&(spade>sliver))
         {
            ResultName = viennamesh::vgmodeler::keys::spade;    // simplex
            type = TTSpade;
         }
         else if (cap>sliver)
         {
            ResultName = viennamesh::vgmodeler::keys::cap;    // simplex
            type = TTCap;
         }
         else
         {
            ResultName = viennamesh::vgmodeler::keys::sliver;    // simplex
            type = TTSliver;
         }
      }
   }
